...
function FillGrid() {    
    ...
    CallService('Client/ClientTaxSetupService.svc', 'GetPage', params, OnClientTaxSetupGetPageSuccess, true);
}
function OnClientTaxSetupGetPageSuccess(dtoClientTaxSetupPage) {    
    if (dtoClientTaxSetupPage != undefined) {
        $('#txtCurrentRecordCount').val(dtoClientTaxSetupPage.ClientTaxSetups.length);
        $("#txtTotalRecords").val(dtoClientTaxSetupPage.TotalRecords);
        var html = '';
        for (var i = 0; i < dtoClientTaxSetupPage.ClientTaxSetups.length; i++) {
            html += '<tr id="' + dtoClientTaxSetupPage.ClientTaxSetups[i].ClientTaxSetupId + '" class="' + (i % 2 == 0 ? "footable-even" : "footable-odd") + '">';  
            html += '<td><input type="checkbox" name="chk"/></td>';
            html += '<td><a href="/Client/Main.html#ViewClientTaxSetup:' + dtoClientTaxSetupPage.ClientTaxSetups[i].ClientTaxSetupId + '" class="link">' + _.escape(FormatDateUTC(dtoClientTaxSetupPage.ClientTaxSetups[i].EffectiveDate)) + '</a></td>';  
            html += '<td>' + dtoClientTaxSetupPage.ClientTaxSetups[i].StateName + '</td>';  
            html += '<td>' + dtoClientTaxSetupPage.ClientTaxSetups[i].SUITaxEmployer.toFixed(6) + '%</td>';  
            html += '<td>' + accounting.formatMoney(dtoClientTaxSetupPage.ClientTaxSetups[i].MinLimit) + '</td>'; 
            html += '<td>' + accounting.formatMoney(dtoClientTaxSetupPage.ClientTaxSetups[i].MaxLimit) + '</td>';                    
            html += ' <td class="td-center"><a href="javascript:void(0)" onclick=\"EditRecord(' + dtoClientTaxSetupPage.ClientTaxSetups[i].ClientTaxSetupId + ')\" class="link"><i class="fa fa-pencil" aria-hidden="true"></i></a>&nbsp;&nbsp;&nbsp;<a href="javascript:void(0)" onclick=\"DeleteRecord(' + dtoClientTaxSetupPage.ClientTaxSetups[i].ClientTaxSetupId + ')\" class="link"><i class="fa fa-trash" aria-hidden="true"></i></a></td>';
            html += '</tr>';
        }
        $('#bodyClientTaxSetups').html(html);
        if (dtoClientTaxSetupPage.TotalRecords > 0) {
...
function NewRecord() {
    window.location = '/Client/Main.html#ClientTaxSetup';
}

function EditRecord(ClientTaxSetupId) {
    window.location = '/Client/Main.html#ClientTaxSetup:' + ClientTaxSetupId;
}

function Delete(ClientTaxSetupId) {
    var params = { ClientId: $.localStorage.get('ClientId'), ClientTaxSetupId: ClientTaxSetupId };
    CallService('Client/ClientTaxSetupService.svc', 'Delete', params, OnClientTaxSetupDeleteSuccess, false);   
}
function OnClientTaxSetupDeleteSuccess(response) { 

}
function DeleteRecord(ClientTaxSetupId) {
    bootbox.dialog({
        ...
        buttons: {
            ok: {
                ...
                callback: function () {
                    var params = { ClientId: $.localStorage.get('ClientId'), ClientTaxSetupId: ClientTaxSetupId };
                    var Deletable = CallService1('Client/ClientTaxSetupService.svc', 'Deletable', params);
                    if (Deletable == true) {
                        Delete(ClientTaxSetupId);
                        $("#txtCurrentPageIndex").val('1');
                        FillGrid();
                    }
                    else {
    ...
    bootbox.dialog({
        ...
        buttons: {
            ok: {
                ...
                className: "btn btn-primary",
                callback: function () {
                    $('#bodyClientTaxSetups').find('tr').each(function () {
                        var row = $(this);
...
FillGrid();
window.scrollTo(0, 0);     
socket.on('GetClientTaxSetupPage', function () {
    FillGrid();
})