...
function FillGrid() {    
    ...
    CallService('Client/LocationService.svc', 'GetPage', params, OnLocationGetPageSuccess, true);
}
function OnLocationGetPageSuccess(dtoLocationPage) {
    if (dtoLocationPage != undefined) {
        $('#txtCurrentRecordCount').val(dtoLocationPage.Locations.length);
        $("#txtTotalRecords").val(_.escape(dtoLocationPage.TotalRecords));
        var html = '';
        for (var i = 0; i < dtoLocationPage.Locations.length; i++) {
            html += '<tr id="' + _.escape(dtoLocationPage.Locations[i].LocationId) + '" class="' + (i % 2 == 0 ? "footable-even" : "footable-odd") + '">';
            html += '<td><input type="checkbox" name="chk"/></td>';
            html += '<td><a href="/Client/Main.html#ViewLocation:' + _.escape(dtoLocationPage.Locations[i].LocationId) + '" class="link">' + _.escape(dtoLocationPage.Locations[i].LocationName) + '</a></td>';  
            html += '<td>' + _.escape(dtoLocationPage.Locations[i].StateName) + '</td>';  
            html += ' <td class="td-center"><a href="javascript:void(0)" onclick=\"EditRecord(' + _.escape(dtoLocationPage.Locations[i].LocationId) + ')\" class="link"><i class="fa fa-pencil" aria-hidden="true"></i></a>&nbsp;&nbsp;&nbsp;<a href="javascript:void(0)" onclick=\"DeleteRecord(' + _.escape(dtoLocationPage.Locations[i].LocationId) + ')\" class="link"><i class="fa fa-trash" aria-hidden="true"></i></a></td>';
            html += '</tr>';
        }
        $('#bodyLocations').html(html);
        if (dtoLocationPage.TotalRecords > 0) {
...
function NewRecord() {
    window.location = '/Client/Main.html#Location';
}

function EditRecord(LocationId) {
    window.location = '/Client/Main.html#Location:' + LocationId;
}

function Delete(LocationId) {
    var params = { ClientId: $.localStorage.get('ClientId'), LocationId: LocationId };
    CallService('Client/LocationService.svc', 'Delete', params, OnLocationDeleteSuccess, false);
}
function OnLocationDeleteSuccess(response) {
    ...
    bootbox.dialog({
        ...
        buttons: {
            ok: {
                ...
                callback: function () {
                    var params = { ClientId: $.localStorage.get('ClientId'), LocationId: LocationId };
                    var Deletable = CallService1('Client/LocationService.svc', 'Deletable', params);
                    if (Deletable == true) {
                        Delete(LocationId);
                        $("#txtCurrentPageIndex").val('1');
                        FillGrid();
    ...
    bootbox.dialog({
        ...
        buttons: {
            ok: {
                ...
                className: "btn btn-primary",
                callback: function () {
                    $('#bodyLocations').find('tr').each(function () {
                        var row = $(this);
...
FillGrid();
window.scrollTo(0, 0);
socket.on('GetLocationPage', function () {
    FillGrid();
})
