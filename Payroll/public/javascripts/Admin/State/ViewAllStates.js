function FillGrid() {    
    ...
    CallService('Admin/StateService.svc', 'GetPage', params, OnStateGetPageSuccess, true);
}
function OnStateGetPageSuccess(dtoStatePage) {    
    if (dtoStatePage != undefined) {
        $('#txtCurrentRecordCount').val(dtoStatePage.States.length);
        $("#txtTotalRecords").val(dtoStatePage.TotalRecords);
        var html = '';
        for (var i = 0; i < dtoStatePage.States.length; i++) {            
            html += '<tr id="' + dtoStatePage.States[i].StateId + '" class="' + (i % 2 == 0 ? "footable-even" : "footable-odd") + '">';  
            html += '<td><input type="checkbox" name="chk"/></td>';
            html += '<td><a href="/Admin/Main.html#ViewState:' + dtoStatePage.States[i].StateId + '" class="link">' + _.escape(dtoStatePage.States[i].StateCode) + '</a></td>';
            html += '<td>' + _.escape(dtoStatePage.States[i].StateName) + '</td>';            
            html += ' <td class="td-center"><a href="javascript:void(0)" onclick=\"EditRecord(' + dtoStatePage.States[i].StateId + ')\" class="link"><i class="fa fa-pencil" aria-hidden="true"></i></a>&nbsp;&nbsp;&nbsp;<a href="javascript:void(0)" onclick=\"DeleteRecord(' + dtoStatePage.States[i].StateId + ')\" class="link"><i class="fa fa-trash" aria-hidden="true"></i></a></td>';
            html += '</tr>';
        }
        $('#bodyStates').html(html);
        if (dtoStatePage.TotalRecords > 0) {
...
function NewRecord() {
    window.location = '/Admin/Main.html#State';
}

function EditRecord(StateId) {
    window.location = '/Admin/Main.html#State:' + StateId;
}

function Delete(StateId) {
    var params = { StateId: StateId };
    CallService('Admin/StateService.svc', 'Delete', params, OnStateDeleteSuccess, false);   
}
function OnStateDeleteSuccess(response) { 
    ...
    bootbox.dialog({
        ...
        buttons: {
            ok: {
                ...
                callback: function () {
                    var params = { StateId: StateId };
                    var Deletable = CallService1('Admin/StateService.svc', 'Deletable', params);
                    if (Deletable == true) {
                        Delete(StateId);
                        $("#txtCurrentPageIndex").val('1');
                        FillGrid();
    ...
    bootbox.dialog({
        ...
        buttons: {
            ok: {
                ...
                className: "btn btn-primary",
                callback: function () {
                    $('#bodyStates').find('tr').each(function () {
                        var row = $(this);
...
FillGrid();
socket.on('GetStatePage', function () {
    FillGrid();
})