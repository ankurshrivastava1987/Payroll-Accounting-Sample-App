CheckPageRequest();
function FillUnApprovedList() {
    var params = { ClientId: parseInt($.localStorage.get('ClientId'), 10)};
    var PayChecks = CallService1('Client/PayCheckService.svc', 'GetUnApprovedPayCheck', params);
    var TotalHours = 0;
    var TotalPay = 0;
    var TotalNetPay = 0;
    var TotalTaxes = 0;
    ...
    for (var i = 0; i < PayChecks.length; i++) {
        ...
        html += '<td><input type="checkbox" name="chk1"/></td>';        
        ...
        html += '<td style="text-align:right">' + accounting.toFixed(PayChecks[i].Hours, 2) + '</td>';
    ...
    html += '<td style="text-align:right">' + accounting.toFixed(TotalHours, 2) + '</td>';
...
function Step(index) {
    if (index == 1) {
        ...
        window.location = '/Client/Main.html#ApprovePaychecks';
    }
    else if (index == 3) {
...
function DeleteSelected() {
    if ($('td input[name="chk1"]:checked').is(":empty") == false) {
        bootbox.dialog({
            ...
            ok: {
                ...
                callback: function () {
                    $('#bodyPayChecks').find('tr').each(function () {
                        var row = $(this);
                        if (row.find('input[name="chk1"]').is(':checked')) {
