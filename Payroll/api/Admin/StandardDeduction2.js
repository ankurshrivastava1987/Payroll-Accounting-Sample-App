exports.GetPage = function (req, res) {
    var dtoStandardDeduction2Page = new Object();
    var connection = new sql.Connection(Configuration.ConnectionString, function (err) {
        if (err) {
            ...
            request.execute('prcStandardDeduction2GetPage', function (err, recordsets, returnValue, affected) {
                connection.close();
                if (err) {
                    winston.log('error', err);
                }
                    ...
                    try {
                        dtoStandardDeduction2Page.StandardDeduction2s = ValidateData(recordsets[0]);
                        dtoStandardDeduction2Page.TotalRecords = recordsets[1][0].TotalRecords;
                        res.json(dtoStandardDeduction2Page);
                    }
                    catch (err) {
                        winston.log('error', err);
                    }
...
exports.Save = function (req, res) {
    var connection = new sql.Connection(Configuration.ConnectionString, function (err) {
        if (err) {
            ...
            request.input('NetAmountMultiplyBy', sql.Decimal(18, 5), query.NetAmountMultiplyBy);
            request.input('StandardDeductions', StandardDeductions);           
            request.execute('prcStandardDeduction2Save', function (err, recordsets, returnValue, affected) {
                connection.close();
                if (err) {
                    winston.log('error', err);
                }
                    ...
                    try {
                        socketio.emit('GetStandardDeduction2Page');
                        if (query.StandardDeductionId > 0) {
                            socketio.emit('GetStandardDeduction2Object', query.StandardDeductionId);
                        }
...
exports.Delete = function (req, res) {
    var connection = new sql.Connection(Configuration.ConnectionString, function (err) {
        if (err) {
            ...
            var query = req.body;
            request.input('StandardDeductionId', sql.BigInt, query.StandardDeductionId);
            request.execute('prcStandardDeduction2Delete', function (err, recordsets, returnValue, affected) {
                connection.close();
                if (err) {
                    winston.log('error', err);
                }
...
exports.Deletable = function (req, res) {
    var connection = new sql.Connection(Configuration.ConnectionString, function (err) {
        if (err) {
            ...
            var query = req.body;
            request.input('StandardDeductionId', sql.BigInt, query.StandardDeductionId);
            request.execute('prcStandardDeduction2Deletable', function (err, recordsets, returnValue, affected) {
                connection.close();
                if (err) {
                    winston.log('error', err);
                }
...
exports.GetObject = function (req, res) {
    var dtoStandardDeduction2 = new Object();
    var connection = new sql.Connection(Configuration.ConnectionString, function (err) {
        if (err) {
            winston.log('error', err);
        }
            ...
            var query = req.body;
            request.input('StandardDeductionId', sql.BigInt, query.StandardDeductionId);
            request.execute('prcStandardDeduction2Get', function (err, recordsets, returnValue, affected) {
                connection.close();
                if (err) {
                    winston.log('error', err);
                }
                    ...
                    try {
                        if (recordsets[0].length > 0) {
                            dtoStandardDeduction2 = ValidateData(recordsets[0])[0];
                        }
                        res.json(dtoStandardDeduction2);
                    }
                    catch (err) {
                        winston.log('error', err);
                    }
...
exports.GetStandardDeduction2Detail = function (req, res) {
    var connection = new sql.Connection(Configuration.ConnectionString, function (err) {
        if (err) {
            winston.log('error', err);
        }
            ...
            var query = req.body;
            request.input('StandardDeductionId', sql.BigInt, query.StandardDeductionId);
            request.execute('prcStandardDeduction2DetailGet', function (err, recordsets, returnValue, affected) {
                connection.close();
                if (err) {
                    winston.log('error', err);
                }
...
function ValidateData(arrData) {
    for (i = 0; i < arrData.length; i++) {
        ...
        arrData[i].NetAmountMultiplyBy = arrData[i].NetAmountMultiplyBy == null ? 0 : arrData[i].NetAmountMultiplyBy;
    }
    return arrData;
}
...
exports.UploadFile = function (req, res) {
    upload(req, res, function (err) {
        ...
        excel(__dirname + '/../../tmp/' + UploadFileName, function (err, arrData) {
            ...
            Import.columns.add('NetAmountMultiplyBy', sql.Decimal(18, 2));
            Import.columns.add('PayScheduleRecurrence', sql.NVarChar(100));
            Import.columns.add('FilingStatus', sql.NVarChar(100));
            ...
            var connection = new sql.Connection(Configuration.ConnectionString, function (err) {
                if (err) {
                    ...
                    request.execute('prcStandardDeduction2BulkSave', function (err, recordsets, returnValue, affected) {
                        connection.close();
                        if (err) {
                            winston.log('error', err);
                        }
