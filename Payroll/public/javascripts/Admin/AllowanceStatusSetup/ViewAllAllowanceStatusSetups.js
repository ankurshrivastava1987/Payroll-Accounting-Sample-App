function FillGrid() {   
    ...
    CallService('Admin/AllowanceStatusSetupService.svc', 'GetPage', params, OnAllowanceStatusSetupGetPageSuccess, true);
}
function OnAllowanceStatusSetupGetPageSuccess(dtoAllowanceStatusSetupPage) {    
    if (dtoAllowanceStatusSetupPage != undefined) {
        $('#txtCurrentRecordCount').val(dtoAllowanceStatusSetupPage.AllowanceStatusSetups.length);
        $("#txtTotalRecords").val(dtoAllowanceStatusSetupPage.TotalRecords);
        var html = '';
        for (var i = 0; i < dtoAllowanceStatusSetupPage.AllowanceStatusSetups.length; i++) {
            html += '<tr id="' + dtoAllowanceStatusSetupPage.AllowanceStatusSetups[i].AllowanceStatusSetupId + '" class="' + (i % 2 == 0 ? "footable-even" : "footable-odd") + '">';  
            html += '<td><input type="checkbox" name="chk"/></td>';
            html += '<td><a href="/Admin/Main.html#ViewAllowanceStatusSetup:' + dtoAllowanceStatusSetupPage.AllowanceStatusSetups[i].AllowanceStatusSetupId + '" class="link">' + _.escape(dtoAllowanceStatusSetupPage.AllowanceStatusSetups[i].StateName) + '</a></td>';
            html += '<td>' + FormatDateUTC(dtoAllowanceStatusSetupPage.AllowanceStatusSetups[i].EffectiveDate) + '</td>';                                  
            html += ' <td class="td-center"><a href="javascript:void(0)" onclick=\"EditRecord(' + dtoAllowanceStatusSetupPage.AllowanceStatusSetups[i].AllowanceStatusSetupId + ')\" class="link"><i class="fa fa-pencil" aria-hidden="true"></i></a>&nbsp;&nbsp;&nbsp;<a href="javascript:void(0)" onclick=\"DeleteRecord(' + dtoAllowanceStatusSetupPage.AllowanceStatusSetups[i].AllowanceStatusSetupId + ')\" class="link"><i class="fa fa-trash" aria-hidden="true"></i></a></td>';
            html += '</tr>';
        }
        $('#bodyAllowanceStatusSetups').html(html);
        if (dtoAllowanceStatusSetupPage.TotalRecords > 0) {
...
function NewRecord() {
    window.location = '/Admin/Main.html#AllowanceStatusSetup';
}

function EditRecord(AllowanceStatusSetupId) {
    window.location = '/Admin/Main.html#AllowanceStatusSetup:' + AllowanceStatusSetupId;
}

function Delete(AllowanceStatusSetupId) {
    var params = { AllowanceStatusSetupId: AllowanceStatusSetupId };
    CallService('Admin/AllowanceStatusSetupService.svc', 'Delete', params, OnAllowanceStatusSetupDeleteSuccess, false);   
}
function OnAllowanceStatusSetupDeleteSuccess(response) { 
    ...
    bootbox.dialog({
        ...
        buttons: {
            ok: {
                ...
                callback: function () {
                    var params = { AllowanceStatusSetupId: AllowanceStatusSetupId };
                    var Deletable = CallService1('Admin/AllowanceStatusSetupService.svc', 'Deletable', params);
                    if (Deletable == true) {
    ...
    bootbox.dialog({
        ...
        buttons: {
            ok: {
                ...
                className: "btn btn-primary",
                callback: function () {
                    $('#bodyAllowanceStatusSetups').find('tr').each(function () {
                        var row = $(this);
...
FillGrid();
socket.on('GetAllowanceStatusSetupPage', function () {
    FillGrid();
})


$('#aDownload').show();
$('#aDownload').attr('href', '/templates/AllowanceStatusSetupTemplate.xlsx');
$('#aUpload').attr('data-bb', 'AllowanceStatusSetup_html');
$('#aUpload').show();