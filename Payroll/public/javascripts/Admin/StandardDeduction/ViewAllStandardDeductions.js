function FillGrid() {    
    ...
    CallService('Admin/StandardDeductionService.svc', 'GetPage', params, OnStandardDeductionGetPageSuccess, true);
}
function OnStandardDeductionGetPageSuccess(dtoStandardDeductionPage) {    
    if (dtoStandardDeductionPage != undefined) {
        $('#txtCurrentRecordCount').val(dtoStandardDeductionPage.StandardDeductions.length);
        $("#txtTotalRecords").val(dtoStandardDeductionPage.TotalRecords);
        var html = '';
        for (var i = 0; i < dtoStandardDeductionPage.StandardDeductions.length; i++) {
            html += '<tr id="' + dtoStandardDeductionPage.StandardDeductions[i].StandardDeductionId + '" class="' + (i % 2 == 0 ? "footable-even" : "footable-odd") + '">';  
            html += '<td><input type="checkbox" name="chk"/></td>';
            html += '<td><a href="/Admin/Main.html#ViewStandardDeduction:' + dtoStandardDeductionPage.StandardDeductions[i].StandardDeductionId + '" class="link">' + _.escape(dtoStandardDeductionPage.StandardDeductions[i].StateName) + '</a></td>';
            html += '<td>' + FormatDateUTC(dtoStandardDeductionPage.StandardDeductions[i].EffectiveDate) + '</td>';                                  
            html += ' <td class="td-center"><a href="javascript:void(0)" onclick=\"EditRecord(' + dtoStandardDeductionPage.StandardDeductions[i].StandardDeductionId + ')\" class="link"><i class="fa fa-pencil" aria-hidden="true"></i></a>&nbsp;&nbsp;&nbsp;<a href="javascript:void(0)" onclick=\"DeleteRecord(' + dtoStandardDeductionPage.StandardDeductions[i].StandardDeductionId + ')\" class="link"><i class="fa fa-trash" aria-hidden="true"></i></a></td>';
            html += '</tr>';
        }
        $('#bodyStandardDeductions').html(html);
        if (dtoStandardDeductionPage.TotalRecords > 0) {
...
function NewRecord() {
    window.location = '/Admin/Main.html#StandardDeduction';
}

function EditRecord(StandardDeductionId) {
    window.location = '/Admin/Main.html#StandardDeduction:' + StandardDeductionId;
}

function Delete(StandardDeductionId) {
    var params = { StandardDeductionId: StandardDeductionId };
    CallService('Admin/StandardDeductionService.svc', 'Delete', params, OnStandardDeductionDeleteSuccess, false);   
}
function OnStandardDeductionDeleteSuccess(response) { 
    ...
    bootbox.dialog({
        ...
        buttons: {
            ok: {
                ...
                callback: function () {
                    var params = { StandardDeductionId: StandardDeductionId };
                    var Deletable = CallService1('Admin/StandardDeductionService.svc', 'Deletable', params);
                    if (Deletable == true) {
...
FillGrid();
socket.on('GetStandardDeductionPage', function () {
    FillGrid();
})


$('#aDownload').show();
$('#aDownload').attr('href', '/templates/StandardDeduction1Template.xlsx');
$('#aUpload').attr('data-bb', 'StandardDeduction1_html');
$('#aUpload').show();