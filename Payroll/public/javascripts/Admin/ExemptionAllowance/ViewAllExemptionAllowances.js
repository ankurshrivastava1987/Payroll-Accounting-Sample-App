function FillGrid() {    
    ...
    CallService('Admin/ExemptionAllowanceService.svc', 'GetPage', params, OnExemptionAllowanceGetPageSuccess, true);
}
function OnExemptionAllowanceGetPageSuccess(dtoExemptionAllowancePage) {    
    if (dtoExemptionAllowancePage != undefined) {
        $('#txtCurrentRecordCount').val(dtoExemptionAllowancePage.ExemptionAllowances.length);
        $("#txtTotalRecords").val(dtoExemptionAllowancePage.TotalRecords);
        var html = '';
        for (var i = 0; i < dtoExemptionAllowancePage.ExemptionAllowances.length; i++) {
            html += '<tr id="' + dtoExemptionAllowancePage.ExemptionAllowances[i].ExemptionAllowanceId + '" class="' + (i % 2 == 0 ? "footable-even" : "footable-odd") + '">';  
            html += '<td><input type="checkbox" name="chk"/></td>';
            html += '<td><a href="/Admin/Main.html#ViewExemptionAllowance:' + dtoExemptionAllowancePage.ExemptionAllowances[i].ExemptionAllowanceId + '" class="link">' + _.escape(dtoExemptionAllowancePage.ExemptionAllowances[i].StateName) + '</a></td>';
            html += '<td>' + FormatDateUTC(dtoExemptionAllowancePage.ExemptionAllowances[i].EffectiveDate) + '</td>';                                  
            html += ' <td class="td-center"><a href="javascript:void(0)" onclick=\"EditRecord(' + dtoExemptionAllowancePage.ExemptionAllowances[i].ExemptionAllowanceId + ')\" class="link"><i class="fa fa-pencil" aria-hidden="true"></i></a>&nbsp;&nbsp;&nbsp;<a href="javascript:void(0)" onclick=\"DeleteRecord(' + dtoExemptionAllowancePage.ExemptionAllowances[i].ExemptionAllowanceId + ')\" class="link"><i class="fa fa-trash" aria-hidden="true"></i></a></td>';
            html += '</tr>';
        }
        $('#bodyExemptionAllowances').html(html);
        if (dtoExemptionAllowancePage.TotalRecords > 0) {
...
function NewRecord() {
    window.location = '/Admin/Main.html#AddExemptionAllowance';
}

function EditRecord(ExemptionAllowanceId) {
    window.location = '/Admin/Main.html#ExemptionAllowance:' + ExemptionAllowanceId;
}

function Delete(ExemptionAllowanceId) {
    var params = { ExemptionAllowanceId: ExemptionAllowanceId };
    CallService('Admin/ExemptionAllowanceService.svc', 'Delete', params, OnExemptionAllowanceDeleteSuccess, false);   
}
function OnExemptionAllowanceDeleteSuccess(response) { 

}
function DeleteRecord(ExemptionAllowanceId) {
    bootbox.dialog({
        ...
        buttons: {
            ok: {
                ...
                callback: function () {
                    var params = { ExemptionAllowanceId: ExemptionAllowanceId };
                    var Deletable = CallService1('Admin/ExemptionAllowanceService.svc', 'Deletable', params);
                    if (Deletable == true) {
                        Delete(ExemptionAllowanceId);
                        $("#txtCurrentPageIndex").val('1');
                        FillGrid();
                    }
                    else {
    ...
    bootbox.dialog({
        ...
        buttons: {
            ok: {
                ...
                className: "btn btn-primary",
                callback: function () {
                    $('#bodyExemptionAllowances').find('tr').each(function () {
                        var row = $(this);
...
FillGrid();
socket.on('GetExemptionAllowancePage', function () {
    FillGrid();
})


$('#aDownload').show();
$('#aDownload').attr('href', '/templates/ExemptionAllowanceTemplate.xlsx');
$('#aUpload').attr('data-bb', 'ExemptionAllowance_html');
$('#aUpload').show();