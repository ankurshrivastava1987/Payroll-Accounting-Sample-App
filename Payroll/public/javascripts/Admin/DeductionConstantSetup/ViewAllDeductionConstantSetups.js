function FillGrid() {    
    ...
    CallService('Admin/DeductionConstantSetupService.svc', 'GetPage', params, OnDeductionConstantSetupGetPageSuccess, true);
}
function OnDeductionConstantSetupGetPageSuccess(dtoDeductionConstantSetupPage) {    
    if (dtoDeductionConstantSetupPage != undefined) {
        $('#txtCurrentRecordCount').val(dtoDeductionConstantSetupPage.DeductionConstantSetups.length);
        $("#txtTotalRecords").val(dtoDeductionConstantSetupPage.TotalRecords);
        var html = '';
        for (var i = 0; i < dtoDeductionConstantSetupPage.DeductionConstantSetups.length; i++) {
            html += '<tr id="' + dtoDeductionConstantSetupPage.DeductionConstantSetups[i].DeductionConstantSetupId + '" class="' + (i % 2 == 0 ? "footable-even" : "footable-odd") + '">';  
            html += '<td><input type="checkbox" name="chk"/></td>';
            html += '<td><a href="/Admin/Main.html#ViewDeductionConstantSetup:' + dtoDeductionConstantSetupPage.DeductionConstantSetups[i].DeductionConstantSetupId + '" class="link">' + _.escape(dtoDeductionConstantSetupPage.DeductionConstantSetups[i].StateName) + '</a></td>';
            html += '<td>' + FormatDateUTC(dtoDeductionConstantSetupPage.DeductionConstantSetups[i].EffectiveDate) + '</td>';                                  
            html += ' <td class="td-center"><a href="javascript:void(0)" onclick=\"EditRecord(' + dtoDeductionConstantSetupPage.DeductionConstantSetups[i].DeductionConstantSetupId + ')\" class="link"><i class="fa fa-pencil" aria-hidden="true"></i></a>&nbsp;&nbsp;&nbsp;<a href="javascript:void(0)" onclick=\"DeleteRecord(' + dtoDeductionConstantSetupPage.DeductionConstantSetups[i].DeductionConstantSetupId + ')\" class="link"><i class="fa fa-trash" aria-hidden="true"></i></a></td>';
            html += '</tr>';
        }
        $('#bodyDeductionConstantSetups').html(html);
        if (dtoDeductionConstantSetupPage.TotalRecords > 0) {
...
function NewRecord() {
    window.location = '/Admin/Main.html#DeductionConstantSetup';
}

function EditRecord(DeductionConstantSetupId) {
    window.location = '/Admin/Main.html#DeductionConstantSetup:' + DeductionConstantSetupId;
}

function Delete(DeductionConstantSetupId) {
    var params = { DeductionConstantSetupId: DeductionConstantSetupId };
    CallService('Admin/DeductionConstantSetupService.svc', 'Delete', params, OnDeductionConstantSetupDeleteSuccess, false);   
}
function OnDeductionConstantSetupDeleteSuccess(response) { 

}
function DeleteRecord(DeductionConstantSetupId) {
    bootbox.dialog({
        ...
        buttons: {
            ok: {
                ...
                callback: function () {
                    var params = { DeductionConstantSetupId: DeductionConstantSetupId };
                    var Deletable = CallService1('Admin/DeductionConstantSetupService.svc', 'Deletable', params);
                    if (Deletable == true) {
                        Delete(DeductionConstantSetupId);
                        $("#txtCurrentPageIndex").val('1');
                        FillGrid();
                    }
                    else {
    ...
    bootbox.dialog({
        ...
        buttons: {
            ok: {
                ...
                className: "btn btn-primary",
                callback: function () {
                    $('#bodyDeductionConstantSetups').find('tr').each(function () {
                        var row = $(this);
...
FillGrid();
socket.on('GetDeductionConstantSetupPage', function () {
    FillGrid();
})

$('#aDownload').show();
$('#aDownload').attr('href', '/templates/DeductionConstantSetupTemplate.xlsx');
$('#aUpload').attr('data-bb', 'DeductionConstantSetup_html');
$('#aUpload').show();