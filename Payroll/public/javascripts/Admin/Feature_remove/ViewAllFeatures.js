function FillGrid() {
    ...
    CallService('Admin/FeatureService.svc', 'GetPage', params, OnFeatureGetPageSuccess, true);
}
function OnFeatureGetPageSuccess(dtoFeaturePage) {    
    if (dtoFeaturePage != undefined) {
        $('#txtCurrentRecordCount').val(dtoFeaturePage.Features.length);
        $("#txtTotalRecords").val(dtoFeaturePage.TotalRecords);
        var html = '';
        for (var i = 0; i < dtoFeaturePage.Features.length; i++) {
            html += '<tr id="' + dtoFeaturePage.Features[i].FeatureId + '" class="' + (i % 2 == 0 ? "footable-even" : "footable-odd") + '">';  
            html += '<td><input type="checkbox" name="chk"/></td>';
            html += '<td><a href="/Admin/Main.html#ViewFeature:' + dtoFeaturePage.Features[i].FeatureId + '">' + dtoFeaturePage.Features[i].FeatureCode + '</a></td>';
            html += '<td>' + dtoFeaturePage.Features[i].FeatureName + '</td>';
            html += '<td>' + dtoFeaturePage.Features[i].DisplayOrder + '</td>';            
            html += ' <td class="td-center"><a href="javascript:void(0)" onclick=\"EditRecord(' + dtoFeaturePage.Features[i].FeatureId + ')\"><span class="zmdi zmdi-edit"></span></a>&nbsp;&nbsp;&nbsp;<a href="javascript:void(0)" onclick=\"DeleteRecord(' + dtoFeaturePage.Features[i].FeatureId + ')\"><span class="zmdi zmdi-close"></span></a></td>';
            html += '</tr>';
        }
        $('#bodyFeatures').html(html);
        if (dtoFeaturePage.TotalRecords > 0) {
...
function NewRecord() {
    window.location = '/Admin/Feature/AddFeature';
}

function EditRecord(FeatureId) {
    window.location = '/Admin/Main.html#Feature:' + FeatureId;
}

function Delete(FeatureId) {
    var params = { FeatureId: FeatureId };
    CallService('Admin/FeatureService.svc', 'Delete', params, OnFeatureDeleteSuccess, false);   
}
function OnFeatureDeleteSuccess(response) { 

}
function DeleteRecord(FeatureId) {
    bootbox.dialog({
        ...
        buttons: {
            ok: {
                ...
                callback: function () {
                    var params = { FeatureId: FeatureId };
                    var Deletable = CallService1('Admin/FeatureService.svc', 'Deletable', params);
                    if (Deletable == true) {
                        Delete(FeatureId);
                        $("#txtCurrentPageIndex").val('1');
                        FillGrid();
    ...
    bootbox.dialog({
        ...
        buttons: {
            ok: {
                ...
                className: "btn btn-primary",
                callback: function () {
                    $('#bodyFeatures').find('tr').each(function () {
                        var row = $(this);
...
FillGrid();
socket.on('GetFeaturePage', function () {
    FillGrid();
})
