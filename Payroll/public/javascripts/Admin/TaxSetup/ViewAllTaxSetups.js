...
function FillGrid() {    
    ...
    CallService('Admin/TaxSetupService.svc', 'GetPage', params, OnTaxSetupGetPageSuccess, true);
}
function OnTaxSetupGetPageSuccess(dtoTaxSetupPage) {    
    if (dtoTaxSetupPage != undefined) {
        $('#txtCurrentRecordCount').val(dtoTaxSetupPage.TaxSetups.length);
        $("#txtTotalRecords").val(dtoTaxSetupPage.TotalRecords);
        var html = '';
        for (var i = 0; i < dtoTaxSetupPage.TaxSetups.length; i++) {
            html += '<tr id="' + dtoTaxSetupPage.TaxSetups[i].TaxSetupId + '" class="' + (i % 2 == 0 ? "footable-even" : "footable-odd") + '">';  
            html += '<td><input type="checkbox" name="chk"/></td>';
            html += '<td><a href="/Admin/Main.html#ViewTaxSetup:' + dtoTaxSetupPage.TaxSetups[i].TaxSetupId + '" class="link">' + _.escape(FormatDateUTC(dtoTaxSetupPage.TaxSetups[i].EffectiveDate)) + '</a></td>';  
            html += '<td>' + dtoTaxSetupPage.TaxSetups[i].SocialSecurityTaxEmployer.toFixed(2) + '%</td>';
            html += '<td>' + dtoTaxSetupPage.TaxSetups[i].SocialSecurityTaxEmployee.toFixed(2) + '%</td>';
            html += '<td>' + dtoTaxSetupPage.TaxSetups[i].MedicareTaxEmployer.toFixed(2) + '%</td>';
            html += '<td>' + dtoTaxSetupPage.TaxSetups[i].MedicareTaxEmployee.toFixed(2) + '%</td>';           
            html += ' <td class="td-center"><a href="javascript:void(0)" onclick=\"EditRecord(' + dtoTaxSetupPage.TaxSetups[i].TaxSetupId + ')\" class="link"><i class="fa fa-pencil" aria-hidden="true"></i></a>&nbsp;&nbsp;&nbsp;<a href="javascript:void(0)" onclick=\"DeleteRecord(' + dtoTaxSetupPage.TaxSetups[i].TaxSetupId + ')\" class="link"><i class="fa fa-trash" aria-hidden="true"></i></a></td>';
            html += '</tr>';
        }
        $('#bodyTaxSetups').html(html);
        if (dtoTaxSetupPage.TotalRecords > 0) {
...
function NewRecord() {
    window.location = '/Admin/Main.html#TaxSetup';
}

function EditRecord(TaxSetupId) {
    window.location = '/Admin/Main.html#TaxSetup:' + TaxSetupId;
}

function Delete(TaxSetupId) {
    var params = { TaxSetupId: TaxSetupId };
    CallService('Admin/TaxSetupService.svc', 'Delete', params, OnTaxSetupDeleteSuccess, false);   
}
function OnTaxSetupDeleteSuccess(response) { 

}
function DeleteRecord(TaxSetupId) {
    bootbox.dialog({
        ...
        buttons: {
            ok: {
                ...
                callback: function () {
                    var params = { TaxSetupId: TaxSetupId };
                    var Deletable = CallService1('Admin/TaxSetupService.svc', 'Deletable', params);
                    if (Deletable == true) {
                        Delete(TaxSetupId);
                        $("#txtCurrentPageIndex").val('1');
                        FillGrid();
    ...
    bootbox.dialog({
        ...
        buttons: {
            ok: {
                ...
                className: "btn btn-primary",
                callback: function () {
                    $('#bodyTaxSetups').find('tr').each(function () {
                        var row = $(this);
...
socket.on('GetTaxSetupPage', function () {
    FillGrid();
})
