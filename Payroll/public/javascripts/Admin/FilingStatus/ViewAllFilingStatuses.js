function FillGrid() {    
    ...
    CallService('Admin/FilingStatusService.svc', 'GetPage', params, OnFilingStatusGetPageSuccess, true);
}
function OnFilingStatusGetPageSuccess(dtoFilingStatusPage) {    
    if (dtoFilingStatusPage != undefined) {
        $('#txtCurrentRecordCount').val(dtoFilingStatusPage.FilingStatuses.length);
        $("#txtTotalRecords").val(dtoFilingStatusPage.TotalRecords);
        var html = '';
        for (var i = 0; i < dtoFilingStatusPage.FilingStatuses.length; i++) {            
            html += '<tr id="' + dtoFilingStatusPage.FilingStatuses[i].FilingStatusId + '" class="' + (i % 2 == 0 ? "footable-even" : "footable-odd") + '">';  
            html += '<td><input type="checkbox" name="chk"/></td>';
            html += '<td><a href="/Admin/Main.html#ViewFilingStatus:' + dtoFilingStatusPage.FilingStatuses[i].FilingStatusId + '" class="link">' + _.escape(dtoFilingStatusPage.FilingStatuses[i].FilingStatusCode) + '</a></td>';
            html += '<td>' + _.escape(dtoFilingStatusPage.FilingStatuses[i].FilingStatusName) + '</td>';   
            html += '<td>' + _.escape(dtoFilingStatusPage.FilingStatuses[i].StateName) + '</td>';          
            html += ' <td class="td-center"><a href="javascript:void(0)" onclick=\"EditRecord(' + dtoFilingStatusPage.FilingStatuses[i].FilingStatusId + ')\" class="link"><i class="fa fa-pencil" aria-hidden="true"></i></a>&nbsp;&nbsp;&nbsp;<a href="javascript:void(0)" onclick=\"DeleteRecord(' + dtoFilingStatusPage.FilingStatuses[i].FilingStatusId + ')\" class="link"><i class="fa fa-trash" aria-hidden="true"></i></a></td>';
            html += '</tr>';
        }
        $('#bodyFilingStatuses').html(html);
        if (dtoFilingStatusPage.TotalRecords > 0) {
...
function NewRecord() {
    window.location = '/Admin/Main.html#FilingStatus';
}

function EditRecord(FilingStatusId) {
    window.location = '/Admin/Main.html#FilingStatus:' + FilingStatusId;
}

function Delete(FilingStatusId) {
    var params = { FilingStatusId: FilingStatusId };
    CallService('Admin/FilingStatusService.svc', 'Delete', params, OnFilingStatusDeleteSuccess, false);   
}
function OnFilingStatusDeleteSuccess(response) { 

}
function DeleteRecord(FilingStatusId) {
    bootbox.dialog({
        ...
        buttons: {
            ok: {
                ...
                callback: function () {
                    var params = { FilingStatusId: FilingStatusId };
                    var Deletable = CallService1('Admin/FilingStatusService.svc', 'Deletable', params);
                    if (Deletable == true) {
                        Delete(FilingStatusId);
                        $("#txtCurrentPageIndex").val('1');
                        FillGrid();
                    }
                    else {
    ...
    bootbox.dialog({
        ...
        buttons: {
            ok: {
                ...
                className: "btn btn-primary",
                callback: function () {
                    $('#bodyFilingStatuses').find('tr').each(function () {
                        var row = $(this);
...
FillGrid();
socket.on('GetFilingStatusPage', function () {
    FillGrid();
})
