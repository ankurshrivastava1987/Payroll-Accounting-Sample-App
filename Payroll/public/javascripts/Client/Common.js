...
function CallService(serviceName, methodName, parameters, successMethod, asyncValue) {
    $.ajax({
        ...
        error: function (a, b, c) {
            ...
            console.log(b);
            console.log(c);
            if ((a.status == 400) || (a.status == 401) || (a.status == 500)) {
...
$('.decimal6').keypress(function (event) {
    return isNumber(event, this)
}).bind('blur', function () {
    var num;
    ...
    num = isNaN(num) ? '0.000000' : num;
    if (num && num < 0) num = num * -1;
    $(this).val(parseFloat(num).toFixed(6));
});
...
//    i.src = 'http://payrollweb.herokuapp.com/assets/images/logo.png?d=' + escape(Date());
//// escape(Date()) is necessary to override possibility of image coming from cache
//}, 30000);
function CheckPageRequest()
{   
    if ((window.location.href.toLowerCase().indexOf('#myprofile') == -1) && (window.location.href.toLowerCase().indexOf('#changepassword') == -1) && (window.location.href.toLowerCase().indexOf('#client') == -1) && (window.location.href.toLowerCase().indexOf('#servicecancellation') == -1)) {
        if (($.localStorage.get('ClientId') == undefined) || ($.localStorage.get('ClientId') == null)) {
