function FillGrid() {    
    ...
    CallService('Admin/CountyService.svc', 'GetPage', params, OnCountyGetPageSuccess, true);
}
function OnCountyGetPageSuccess(dtoCountyPage) {
    if (dtoCountyPage != undefined) {
        $('#txtCurrentRecordCount').val(dtoCountyPage.Counties.length);
        $("#txtTotalRecords").val(dtoCountyPage.TotalRecords);
        var html = '';
        for (var i = 0; i < dtoCountyPage.Counties.length; i++) {
            html += '<tr id="' + dtoCountyPage.Counties[i].CountyId + '" class="' + (i % 2 == 0 ? "footable-even" : "footable-odd") + '">';
            html += '<td><input type="checkbox" name="chk"/></td>';
            html += '<td><a href="/Admin/Main.html#ViewCounty:' + dtoCountyPage.Counties[i].CountyId + '" class="link">' + _.escape(dtoCountyPage.Counties[i].CountyCode) + '</a></td>';  
            html += '<td>' + _.escape(dtoCountyPage.Counties[i].CountyName) + '</td>'; 
            html += '<td>' + _.escape(dtoCountyPage.Counties[i].StateName) + '</td>';  
            html += ' <td class="td-center"><a href="javascript:void(0)" onclick=\"EditRecord(' + dtoCountyPage.Counties[i].CountyId + ')\" class="link"><i class="fa fa-pencil" aria-hidden="true"></i></a>&nbsp;&nbsp;&nbsp;<a href="javascript:void(0)" onclick=\"DeleteRecord(' + dtoCountyPage.Counties[i].CountyId + ')\" class="link"><i class="fa fa-trash" aria-hidden="true"></i></a></td>';
            html += '</tr>';
        }
        $('#bodyCounties').html(html);
        if (dtoCountyPage.TotalRecords > 0) {
...
function NewRecord() {
    window.location = '/Admin/Main.html#County';
}

function EditRecord(CountyId) {
    window.location = '/Admin/Main.html#County:' + CountyId;
}

function Delete(CountyId) {
    var params = { CountyId: CountyId };
    CallService('Admin/CountyService.svc', 'Delete', params, OnCountyDeleteSuccess, false);
}
function OnCountyDeleteSuccess(response) {

}
function DeleteRecord(CountyId) {
    bootbox.dialog({
        ...
        buttons: {
            ok: {
                ...
                callback: function () {
                    var params = { CountyId: CountyId };
                    var Deletable = CallService1('Admin/CountyService.svc', 'Deletable', params);
                    if (Deletable == true) {
                        Delete(CountyId);
                        $("#txtCurrentPageIndex").val('1');
                        FillGrid();
    ...
    bootbox.dialog({
        ...
        buttons: {
            ok: {
                ...
                className: "btn btn-primary",
                callback: function () {
                    $('#bodyCounties').find('tr').each(function () {
                        var row = $(this);
...
FillGrid();
socket.on('GetCountyPage', function () {
    FillGrid();
})