...
function FillGrid() {    
    ...
    CallService('Client/DepartmentService.svc', 'GetPage', params, OnDepartmentGetPageSuccess, true);
}
function OnDepartmentGetPageSuccess(dtoDepartmentPage) {
    if (dtoDepartmentPage != undefined) {
        $('#txtCurrentRecordCount').val(dtoDepartmentPage.Departments.length);
        $("#txtTotalRecords").val(_.escape(dtoDepartmentPage.TotalRecords));
        var html = '';
        for (var i = 0; i < dtoDepartmentPage.Departments.length; i++) {
            html += '<tr id="' + _.escape(dtoDepartmentPage.Departments[i].DepartmentId) + '" class="' + (i % 2 == 0 ? "footable-even" : "footable-odd") + '">';
            html += '<td><input type="checkbox" name="chk"/></td>';
            html += '<td><a href="/Client/Main.html#ViewDepartment:' + _.escape(dtoDepartmentPage.Departments[i].DepartmentId) + '" class="link">' + _.escape(dtoDepartmentPage.Departments[i].DepartmentName) + '</a></td>';               
            html += ' <td class="td-center"><a href="javascript:void(0)" onclick=\"EditRecord(' + _.escape(dtoDepartmentPage.Departments[i].DepartmentId) + ')\" class="link"><i class="fa fa-pencil" aria-hidden="true"></i></a>&nbsp;&nbsp;&nbsp;<a href="javascript:void(0)" onclick=\"DeleteRecord(' + _.escape(dtoDepartmentPage.Departments[i].DepartmentId) + ')\" class="link"><i class="fa fa-trash" aria-hidden="true"></i></a></td>';
            html += '</tr>';
        }
        $('#bodyDepartments').html(html);
        if (dtoDepartmentPage.TotalRecords > 0) {
...
function NewRecord() {
    window.location = '/Client/Main.html#Department';
}

function EditRecord(DepartmentId) {
    window.location = '/Client/Main.html#Department:' + DepartmentId;
}

function Delete(DepartmentId) {
    var params = { ClientId: $.localStorage.get('ClientId'), DepartmentId: DepartmentId };
    CallService('Client/DepartmentService.svc', 'Delete', params, OnDepartmentDeleteSuccess, false);
}
function OnDepartmentDeleteSuccess(response) {
    ...
    bootbox.dialog({
        ...
        buttons: {
            ok: {
                ...
                callback: function () {
                    var params = { ClientId: $.localStorage.get('ClientId'), DepartmentId: DepartmentId };
                    var Deletable = CallService1('Client/DepartmentService.svc', 'Deletable', params);
                    if (Deletable == true) {
                        Delete(DepartmentId);
                        $("#txtCurrentPageIndex").val('1');
                        FillGrid();
    ...
    bootbox.dialog({
        ...
        buttons: {
            ok: {
                ...
                className: "btn btn-primary",
                callback: function () {
                    $('#bodyDepartments').find('tr').each(function () {
                        var row = $(this);
...
FillGrid();
window.scrollTo(0, 0); 
socket.on('GetDepartmentPage', function () {
    FillGrid();
})