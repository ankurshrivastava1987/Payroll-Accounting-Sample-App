function FillGrid() {    
    ...
    CallService('Admin/AllowanceChartSetupService.svc', 'GetPage', params, OnAllowanceChartSetupGetPageSuccess, true);
}
function OnAllowanceChartSetupGetPageSuccess(dtoAllowanceChartSetupPage) {    
    if (dtoAllowanceChartSetupPage != undefined) {
        $('#txtCurrentRecordCount').val(dtoAllowanceChartSetupPage.AllowanceChartSetups.length);
        $("#txtTotalRecords").val(dtoAllowanceChartSetupPage.TotalRecords);
        var html = '';
        for (var i = 0; i < dtoAllowanceChartSetupPage.AllowanceChartSetups.length; i++) {
            html += '<tr id="' + dtoAllowanceChartSetupPage.AllowanceChartSetups[i].AllowanceChartSetupId + '" class="' + (i % 2 == 0 ? "footable-even" : "footable-odd") + '">';  
            html += '<td><input type="checkbox" name="chk"/></td>';
            html += '<td><a href="/Admin/Main.html#ViewAllowanceChartSetup:' + dtoAllowanceChartSetupPage.AllowanceChartSetups[i].AllowanceChartSetupId + '" class="link">' + _.escape(dtoAllowanceChartSetupPage.AllowanceChartSetups[i].StateName) + '</a></td>';
            html += '<td>' + FormatDateUTC(dtoAllowanceChartSetupPage.AllowanceChartSetups[i].EffectiveDate) + '</td>';                                  
            html += ' <td class="td-center"><a href="javascript:void(0)" onclick=\"EditRecord(' + dtoAllowanceChartSetupPage.AllowanceChartSetups[i].AllowanceChartSetupId + ')\" class="link"><i class="fa fa-pencil" aria-hidden="true"></i></a>&nbsp;&nbsp;&nbsp;<a href="javascript:void(0)" onclick=\"DeleteRecord(' + dtoAllowanceChartSetupPage.AllowanceChartSetups[i].AllowanceChartSetupId + ')\" class="link"><i class="fa fa-trash" aria-hidden="true"></i></a></td>';
            html += '</tr>';
        }
        $('#bodyAllowanceChartSetups').html(html);
        if (dtoAllowanceChartSetupPage.TotalRecords > 0) {
...
function NewRecord() {
    window.location = '/Admin/Main.html#AllowanceChartSetup';
}

function EditRecord(AllowanceChartSetupId) {
    window.location = '/Admin/Main.html#AllowanceChartSetup:' + AllowanceChartSetupId;
}

function Delete(AllowanceChartSetupId) {
    var params = { AllowanceChartSetupId: AllowanceChartSetupId };
    CallService('Admin/AllowanceChartSetupService.svc', 'Delete', params, OnAllowanceChartSetupDeleteSuccess, false);   
}
function OnAllowanceChartSetupDeleteSuccess(response) { 

}
function DeleteRecord(AllowanceChartSetupId) {
    bootbox.dialog({
        ...
        buttons: {
            ok: {
                ...
                callback: function () {
                    var params = { AllowanceChartSetupId: AllowanceChartSetupId };
                    var Deletable = CallService1('Admin/AllowanceChartSetupService.svc', 'Deletable', params);
                    if (Deletable == true) {
                        Delete(AllowanceChartSetupId);
                        $("#txtCurrentPageIndex").val('1');
                        FillGrid();
                    }
                    else {
    ...
    bootbox.dialog({
        ...
        buttons: {
            ok: {
                ...
                className: "btn btn-primary",
                callback: function () {
                    $('#bodyAllowanceChartSetups').find('tr').each(function () {
                        var row = $(this);
            ...
            cancel: {
                label: "<i class=\"fa fa-check\" aria-hidden=\"true\"></i> Cancel",
                className: "btn btn-primary",
                callback: function () {                    
                }
            }
...
FillGrid();
socket.on('GetAllowanceChartSetupPage', function () {
    FillGrid();
})

$('#aDownload').show();
$('#aDownload').attr('href', '/templates/AllowanceChartSetupTemplate.xlsx');
$('#aUpload').attr('data-bb', 'AllowanceChartSetup_html');
$('#aUpload').show();