function FillGrid() {   
    ...
    CallService('Admin/FederalTaxSetupService.svc', 'GetPage', params, OnFederalTaxSetupGetPageSuccess, true);
}
function OnFederalTaxSetupGetPageSuccess(dtoFederalTaxSetupPage) {    
    if (dtoFederalTaxSetupPage != undefined) {
        $('#txtCurrentRecordCount').val(dtoFederalTaxSetupPage.FederalTaxSetups.length);
        $("#txtTotalRecords").val(dtoFederalTaxSetupPage.TotalRecords);
        var html = '';
        for (var i = 0; i < dtoFederalTaxSetupPage.FederalTaxSetups.length; i++) {
            html += '<tr id="' + dtoFederalTaxSetupPage.FederalTaxSetups[i].FederalTaxSetupId + '" class="' + (i % 2 == 0 ? "footable-even" : "footable-odd") + '">';  
            html += '<td><input type="checkbox" name="chk"/></td>';
            html += '<td><a href="/Admin/Main.html#ViewFederalTaxSetup:' + dtoFederalTaxSetupPage.FederalTaxSetups[i].FederalTaxSetupId + '" class="link">' + FormatDateUTC(dtoFederalTaxSetupPage.FederalTaxSetups[i].EffectiveDate) + '</a></td>';
            html += '<td>' + _.escape(dtoFederalTaxSetupPage.FederalTaxSetups[i].FederalFilingStatusName) + '</td>';
            html += '<td>' + _.escape(dtoFederalTaxSetupPage.FederalTaxSetups[i].PayScheduleRecurrenceName) + '</td>';
            html += '<td class="text-right">$' + dtoFederalTaxSetupPage.FederalTaxSetups[i].Allowance.toFixed(2) + '</td>';            
            html += ' <td class="td-center"><a href="javascript:void(0)" onclick=\"EditRecord(' + dtoFederalTaxSetupPage.FederalTaxSetups[i].FederalTaxSetupId + ')\" class="link"><i class="fa fa-pencil" aria-hidden="true"></i></a>&nbsp;&nbsp;&nbsp;<a href="javascript:void(0)" onclick=\"DeleteRecord(' + dtoFederalTaxSetupPage.FederalTaxSetups[i].FederalTaxSetupId + ')\" class="link"><i class="fa fa-trash" aria-hidden="true"></i></a></td>';
            html += '</tr>';
        }
        $('#bodyFederalTaxSetups').html(html);
        if (dtoFederalTaxSetupPage.TotalRecords > 0) {
...
function NewRecord() {
    window.location = '/Admin/Main.html#FederalTaxSetup';
}

function EditRecord(FederalTaxSetupId) {
    window.location = '/Admin/Main.html#FederalTaxSetup:' + FederalTaxSetupId;
}

function Delete(FederalTaxSetupId) {
    var params = { FederalTaxSetupId: FederalTaxSetupId };
    CallService('Admin/FederalTaxSetupService.svc', 'Delete', params, OnFederalTaxSetupDeleteSuccess, false);   
}
function OnFederalTaxSetupDeleteSuccess(response) { 

}
function DeleteRecord(FederalTaxSetupId) {
    bootbox.dialog({
        ...
        buttons: {
            ok: {
                ...
                callback: function () {
                    var params = { FederalTaxSetupId: FederalTaxSetupId };
                    var Deletable = CallService1('Admin/FederalTaxSetupService.svc', 'Deletable', params);
                    if (Deletable == true) {
                        Delete(FederalTaxSetupId);
                        $("#txtCurrentPageIndex").val('1');
                        FillGrid();
                    }
                    else {
    ...
    bootbox.dialog({
        ...
        buttons: {
            ok: {
                ...
                className: "btn btn-primary",
                callback: function () {
                    $('#bodyFederalTaxSetups').find('tr').each(function () {
                        var row = $(this);
...
FillGrid();
socket.on('GetFederalTaxSetupPage', function () {
    FillGrid();
})
$('#aDownload').show();
$('#aDownload').attr('href', '/templates/FederalTaxUploadTemplate.xlsx');
$('#aUpload').show();
$('#aUpload').attr('data-bb', 'FederalTaxSetup_html');