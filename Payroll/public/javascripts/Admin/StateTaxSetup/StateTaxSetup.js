app.page("StateTaxSetup", function () {
    return function (params) {
        if (parseInt(params, 10) > 0) {
            $('#hdnStateTaxSetupId').val(params);
            $('.page-head').html("Edit State Tax Setup");
            DisplayData();
            FillStateTaxSetupDetail();
        }       
    }
});
$('#txtSearch').hide();
...
function OnStateTaxSetupGetObjectSuccess(dtoStateTaxSetup) {
    if (dtoStateTaxSetup.StateTaxSetupId != undefined) {
        $('#cmbStateName').val(dtoStateTaxSetup.StateId);
        $('#cmbStateName').change();
        $('#txtEffectiveDate').val(FormatDateUTC(dtoStateTaxSetup.EffectiveDate));
        $('#cmbPayScheduleRecurrenceName').val(dtoStateTaxSetup.PayScheduleRecurrenceId);
        $('#cmbFilingStatusName').val(dtoStateTaxSetup.TaxStatusId);
    }
    else {
...
function Save() {
    if (form.valid()) {       
        var params = { StateTaxSetupId: parseInt($("#hdnStateTaxSetupId").val(), 10), StateId: parseInt($("#cmbStateName").val(), 10), EffectiveDate: $("#txtEffectiveDate").val(), PayScheduleRecurrenceId: parseInt($("#cmbPayScheduleRecurrenceName").val(), 10), TaxStatusId: parseInt($("#cmbFilingStatusName").val(), 10), StateTaxSetupDetails: $("#hdnStateTaxSetupDetails").val() };
        CallService('Admin/StateTaxSetupService.svc', 'Save', params, OnStateTaxSetupSaveSuccess, false);
    }
};

function OnStateTaxSetupSaveSuccess(StateTaxSetupId) {
...
function DeleteSelected() {
    ...
    $('#tblStateTaxSetupDetails').find('tr').each(function () {
        var row = $(this);
