...
var params = { ClientId: $.localStorage.get('ClientId'), PayStubCode: 'BONUS' };
...
function OnEmployeeGetByPayStubCodeSuccess(Employees) {
    if (Employees != undefined) { 
        ...
        for (var j = 0; j < PayStubs.length; j++) {
            html += '<th ' + (PayStubs[j].PayStubCode == 'BONUS' ? '' : 'class="hide"') + '>' + _.escape(PayStubs[j].PayStubName) + '</th>';
        }
        ...
        for (var i = 0; i < Employees.length; i++) {
            ...
            html += '<td class="hide" style="text-align:right;"><input id="' + Employees[i].EmployeeId + '_-19" class="form-control decimal" style="text-align:right;width:80px;" value="$0.00"/></td>';               
...
function CalculateNetAmount(EmployeeId) {
    ...
    if ($('#chkNetToGross').is(':checked')) {        
        $('#' + EmployeeId + '_-19').val(accounting.formatMoney(parseFloat($('#' + EmployeeId + '_-7').val().replace('$', '').replace(',', '')) + parseFloat($('#' + EmployeeId + '_-8').val().replace('$', '').replace(',', '')) + parseFloat($('#' + EmployeeId + '_-9').val().replace('$', '').replace(',', '')) + parseFloat($('#' + EmployeeId + '_-10').val().replace('$', '').replace(',', '')) + parseFloat($('#' + EmployeeId + '_-11').val().replace('$', '').replace(',', ''))));
    }
    else {
        $('#' + EmployeeId + '_-19').val('$0.00');
    }
}
