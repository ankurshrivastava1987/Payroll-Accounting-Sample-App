function FillGrid() {    
    ...
    CallService('Admin/IndustryService.svc', 'GetPage', params, OnIndustryGetPageSuccess, true);
}
function OnIndustryGetPageSuccess(dtoIndustryPage) {    
    if (dtoIndustryPage != undefined) {
        $('#txtCurrentRecordCount').val(dtoIndustryPage.Industries.length);
        $("#txtTotalRecords").val(dtoIndustryPage.TotalRecords);
        var html = '';
        for (var i = 0; i < dtoIndustryPage.Industries.length; i++) {
            html += '<tr id="' + dtoIndustryPage.Industries[i].IndustryId + '" class="' + (i % 2 == 0 ? "footable-even" : "footable-odd") + '">';  
            html += '<td><input type="checkbox" name="chk"/></td>';
            html += '<td><a href="/Admin/Main.html#ViewIndustry:' + dtoIndustryPage.Industries[i].IndustryId + '" class="link">' + _.escape(dtoIndustryPage.Industries[i].IndustryName) + '</a></td>';            
            html += ' <td class="td-center"><a href="javascript:void(0)" onclick=\"EditRecord(' + dtoIndustryPage.Industries[i].IndustryId + ')\" class="link"><i class="fa fa-pencil" aria-hidden="true"></i></a>&nbsp;&nbsp;&nbsp;<a href="javascript:void(0)" onclick=\"DeleteRecord(' + dtoIndustryPage.Industries[i].IndustryId + ')\" class="link"><i class="fa fa-trash" aria-hidden="true"></i></a></td>';
            html += '</tr>';
        }
        $('#bodyIndustries').html(html);
        if (dtoIndustryPage.TotalRecords > 0) {
...
function NewRecord() {
    window.location = '/Admin/Main.html#Industry';
}

function EditRecord(IndustryId) {
    window.location = '/Admin/Main.html#Industry:' + IndustryId;
}

function Delete(IndustryId) {
    var params = { IndustryId: IndustryId };
    CallService('Admin/IndustryService.svc', 'Delete', params, OnIndustryDeleteSuccess, false);   
}
function OnIndustryDeleteSuccess(response) { 

}
function DeleteRecord(IndustryId) {
    bootbox.dialog({
        ...
        buttons: {
            ok: {
                ...
                callback: function () {
                    var params = { IndustryId: IndustryId };
                    var Deletable = CallService1('Admin/IndustryService.svc', 'Deletable', params);
                    if (Deletable == true) {
                        Delete(IndustryId);
                        $("#txtCurrentPageIndex").val('1');
                        FillGrid();
    ...
    bootbox.dialog({
        ...
        buttons: {
            ok: {
                ...
                className: "btn btn-primary",
                callback: function () {
                    $('#bodyIndustries').find('tr').each(function () {
                        var row = $(this);
...
socket.on('GetIndustryPage', function () {
    FillGrid();
})
