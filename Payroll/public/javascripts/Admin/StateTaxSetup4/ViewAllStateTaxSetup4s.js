function FillGrid() {    
    ...
    CallService('Admin/StateTaxSetup4Service.svc', 'GetPage', params, OnStateTaxSetup4GetPageSuccess, true);
}
function OnStateTaxSetup4GetPageSuccess(dtoStateTaxSetup4Page) {    
    if (dtoStateTaxSetup4Page != undefined) {
        $('#txtCurrentRecordCount').val(dtoStateTaxSetup4Page.StateTaxSetup4s.length);
        $("#txtTotalRecords").val(dtoStateTaxSetup4Page.TotalRecords);
        var html = '';
        for (var i = 0; i < dtoStateTaxSetup4Page.StateTaxSetup4s.length; i++) {
            html += '<tr id="' + dtoStateTaxSetup4Page.StateTaxSetup4s[i].StateTaxSetupId + '" class="' + (i % 2 == 0 ? "footable-even" : "footable-odd") + '">';  
            html += '<td><input type="checkbox" name="chk"/></td>';
            html += '<td><a href="/Admin/Main.html#ViewStateTaxSetup4:' + dtoStateTaxSetup4Page.StateTaxSetup4s[i].StateTaxSetupId + '" class="link">' + _.escape(dtoStateTaxSetup4Page.StateTaxSetup4s[i].StateName) + '</a></td>';
            html += '<td>' + FormatDateUTC(dtoStateTaxSetup4Page.StateTaxSetup4s[i].EffectiveDate) + '</td>';
            html += '<td>' + _.escape(dtoStateTaxSetup4Page.StateTaxSetup4s[i].PayScheduleRecurrenceName) + '</td>';
            html += '<td>' + _.escape(dtoStateTaxSetup4Page.StateTaxSetup4s[i].FilingStatusName) + '</td>';
            html += '<td>' + dtoStateTaxSetup4Page.StateTaxSetup4s[i].LocalTaxPercentage.toFixed(2) + '%' + '</td>';
            html += '<td>' + (dtoStateTaxSetup4Page.StateTaxSetup4s[i].IsDelawareWorkplace== 'Y'? 'Yes': 'No') + '</td>';                                    
            html += ' <td class="td-center"><a href="javascript:void(0)" onclick=\"EditRecord(' + dtoStateTaxSetup4Page.StateTaxSetup4s[i].StateTaxSetupId + ')\" class="link"><i class="fa fa-pencil" aria-hidden="true"></i></a>&nbsp;&nbsp;&nbsp;<a href="javascript:void(0)" onclick=\"DeleteRecord(' + dtoStateTaxSetup4Page.StateTaxSetup4s[i].StateTaxSetupId + ')\" class="link"><i class="fa fa-trash" aria-hidden="true"></i></a></td>';
            html += '</tr>';
        }
        $('#bodyStateTaxSetups').html(html);
        if (dtoStateTaxSetup4Page.TotalRecords > 0) {
...
function NewRecord() {
    window.location = '/Admin/Main.html#StateTaxSetup4';
}

function EditRecord(StateTaxSetupId) {
    window.location = '/Admin/Main.html#StateTaxSetup4:' + StateTaxSetupId;
}

function Delete(StateTaxSetupId) {
    var params = { StateTaxSetupId: StateTaxSetupId };
    CallService('Admin/StateTaxSetup4Service.svc', 'Delete', params, OnStateTaxSetup4DeleteSuccess, false);   
}
function OnStateTaxSetup4DeleteSuccess(response) { 
    ...
    bootbox.dialog({
        ...
        buttons: {
            ok: {
                ...
                callback: function () {
                    var params = { StateTaxSetupId: StateTaxSetupId };
                    var Deletable = CallService1('Admin/StateTaxSetup4Service.svc', 'Deletable', params);
                    if (Deletable == true) {
    ...
    bootbox.dialog({
        ...
        buttons: {
            ok: {
                ...
                className: "btn btn-primary",
                callback: function () {
                    $('#bodyStateTaxSetups').find('tr').each(function () {
                        var row = $(this);
...
FillGrid();
socket.on('GetStateTaxSetup4Page', function () {
    FillGrid();
})

$('#aDownload').show();
$('#aDownload').attr('href', '/templates/StateTaxSetup4UploadTemplate.xlsx');
$('#aUpload').attr('data-bb', 'StateTaxSetup4_html');
$('#aUpload').show();