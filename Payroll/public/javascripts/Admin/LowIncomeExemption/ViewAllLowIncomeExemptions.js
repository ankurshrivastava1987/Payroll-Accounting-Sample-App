function FillGrid() {    
    ...
    CallService('Admin/LowIncomeExemptionService.svc', 'GetPage', params, OnLowIncomeExemptionGetPageSuccess, true);
}
function OnLowIncomeExemptionGetPageSuccess(dtoLowIncomeExemptionPage) {    
    if (dtoLowIncomeExemptionPage != undefined) {
        $('#txtCurrentRecordCount').val(dtoLowIncomeExemptionPage.LowIncomeExemptions.length);
        $("#txtTotalRecords").val(dtoLowIncomeExemptionPage.TotalRecords);
        var html = '';
        for (var i = 0; i < dtoLowIncomeExemptionPage.LowIncomeExemptions.length; i++) {
            html += '<tr id="' + dtoLowIncomeExemptionPage.LowIncomeExemptions[i].LowIncomeExemptionId + '" class="' + (i % 2 == 0 ? "footable-even" : "footable-odd") + '">';  
            html += '<td><input type="checkbox" name="chk"/></td>';
            html += '<td><a href="/Admin/Main.html#ViewLowIncomeExemption:' + dtoLowIncomeExemptionPage.LowIncomeExemptions[i].LowIncomeExemptionId + '" class="link">' + _.escape(dtoLowIncomeExemptionPage.LowIncomeExemptions[i].StateName) + '</a></td>';
            html += '<td>' + FormatDateUTC(dtoLowIncomeExemptionPage.LowIncomeExemptions[i].EffectiveDate) + '</td>';                                  
            html += ' <td class="td-center"><a href="javascript:void(0)" onclick=\"EditRecord(' + dtoLowIncomeExemptionPage.LowIncomeExemptions[i].LowIncomeExemptionId + ')\" class="link"><i class="fa fa-pencil" aria-hidden="true"></i></a>&nbsp;&nbsp;&nbsp;<a href="javascript:void(0)" onclick=\"DeleteRecord(' + dtoLowIncomeExemptionPage.LowIncomeExemptions[i].LowIncomeExemptionId + ')\" class="link"><i class="fa fa-trash" aria-hidden="true"></i></a></td>';
            html += '</tr>';
        }
        $('#bodyLowIncomeExemptions').html(html);
        if (dtoLowIncomeExemptionPage.TotalRecords > 0) {
...
function NewRecord() {
    window.location = '/Admin/Main.html#LowIncomeExemption';
}

function EditRecord(LowIncomeExemptionId) {
    window.location = '/Admin/Main.html#LowIncomeExemption:' + LowIncomeExemptionId;
}

function Delete(LowIncomeExemptionId) {
    var params = { LowIncomeExemptionId: LowIncomeExemptionId };
    CallService('Admin/LowIncomeExemptionService.svc', 'Delete', params, OnLowIncomeExemptionDeleteSuccess, false);   
}
function OnLowIncomeExemptionDeleteSuccess(response) { 

}
function DeleteRecord(LowIncomeExemptionId) {
    bootbox.dialog({
        ...
        buttons: {
            ok: {
                ...
                callback: function () {
                    var params = { LowIncomeExemptionId: LowIncomeExemptionId };
                    var Deletable = CallService1('Admin/LowIncomeExemptionService.svc', 'Deletable', params);
                    if (Deletable == true) {
                        Delete(LowIncomeExemptionId);
                        $("#txtCurrentPageIndex").val('1');
                        FillGrid();
                    }
                    else {
    ...
    bootbox.dialog({
        ...
        buttons: {
            ok: {
                ...
                className: "btn btn-primary",
                callback: function () {
                    $('#bodyLowIncomeExemptions').find('tr').each(function () {
                        var row = $(this);
...
FillGrid();
socket.on('GetLowIncomeExemptionPage', function () {
    FillGrid();
})

$('#aDownload').show();
$('#aDownload').attr('href', '/templates/LowIncomeExemptionTemplate.xlsx');
$('#aUpload').attr('data-bb', 'LowIncomeExemption_html');
$('#aUpload').show();