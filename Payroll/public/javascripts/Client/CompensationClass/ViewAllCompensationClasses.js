...
function FillGrid() {   
    ...
    CallService('Client/CompensationClassService.svc', 'GetPage', params, OnCompensationClassGetPageSuccess, true);
}
function OnCompensationClassGetPageSuccess(dtoCompensationClassPage) {
    if (dtoCompensationClassPage != undefined) {
        $('#txtCurrentRecordCount').val(dtoCompensationClassPage.CompensationClasses.length);
        $("#txtTotalRecords").val(_.escape(dtoCompensationClassPage.TotalRecords));
        var html = '';
        for (var i = 0; i < dtoCompensationClassPage.CompensationClasses.length; i++) {
            html += '<tr id="' + _.escape(dtoCompensationClassPage.CompensationClasses[i].CompensationClassId) + '" class="' + (i % 2 == 0 ? "footable-even" : "footable-odd") + '">';
            html += '<td><input type="checkbox" name="chk"/></td>';
            html += '<td><a href="/Client/Main.html#ViewCompensationClass:' + _.escape(dtoCompensationClassPage.CompensationClasses[i].CompensationClassId) + '" class="link">' + _.escape(dtoCompensationClassPage.CompensationClasses[i].CompensationClassName) + '</a></td>';               
            html += ' <td class="td-center"><a href="javascript:void(0)" onclick=\"EditRecord(' + _.escape(dtoCompensationClassPage.CompensationClasses[i].CompensationClassId) + ')\" class="link"><i class="fa fa-pencil" aria-hidden="true"></i></a>&nbsp;&nbsp;&nbsp;<a href="javascript:void(0)" onclick=\"DeleteRecord(' + _.escape(dtoCompensationClassPage.CompensationClasses[i].CompensationClassId) + ')\" class="link"><i class="fa fa-trash" aria-hidden="true"></i></a></td>';
            html += '</tr>';
        }
        $('#bodyCompensationClasses').html(html);
        if (dtoCompensationClassPage.TotalRecords > 0) {
...
function NewRecord() {
    window.location = '/Client/Main.html#CompensationClass';
}

function EditRecord(CompensationClassId) {
    window.location = '/Client/Main.html#CompensationClass:' + CompensationClassId;
}

function Delete(CompensationClassId) {
    var params = { ClientId: $.localStorage.get('ClientId'), CompensationClassId: CompensationClassId };
    CallService('Client/CompensationClassService.svc', 'Delete', params, OnCompensationClassDeleteSuccess, false);
}
function OnCompensationClassDeleteSuccess(response) {

}
function DeleteRecord(CompensationClassId) {
    bootbox.dialog({
        ...
        buttons: {
            ok: {
                ...
                callback: function () {
                    var params = { ClientId: $.localStorage.get('ClientId'), CompensationClassId: CompensationClassId };
                    var Deletable = CallService1('Client/CompensationClassService.svc', 'Deletable', params);
                    if (Deletable == true) {
                        Delete(CompensationClassId);
                        $("#txtCurrentPageIndex").val('1');
                        FillGrid();
                    }
                    else {
    ...
    bootbox.dialog({
        ...
        buttons: {
            ok: {
                ...
                className: "btn btn-primary",
                callback: function () {
                    $('#bodyCompensationClasses').find('tr').each(function () {
                        var row = $(this);
...
FillGrid();
window.scrollTo(0, 0); 
socket.on('GetCompensationClassPage', function () {
    FillGrid();
})
