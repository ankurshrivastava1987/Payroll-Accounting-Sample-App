function FillGrid() {    
    ...
    CallService('Admin/UserService.svc', 'GetPage', params, OnUserGetPageSuccess, true);
}
function OnUserGetPageSuccess(dtoUserPage) {    
    if (dtoUserPage != undefined) {
        $('#txtCurrentRecordCount').val(dtoUserPage.Users.length);
        $("#txtTotalRecords").val(dtoUserPage.TotalRecords);
        var html = '';
        for (var i = 0; i < dtoUserPage.Users.length; i++) {
            html += '<tr id="' + dtoUserPage.Users[i].UserId + '" class="' + (i % 2 == 0 ? "footable-even" : "footable-odd") + '">';  
            html += '<td><input type="checkbox" name="chk"/></td>';
            html += '<td><a href="/Admin/Main.html#ViewUser:' + dtoUserPage.Users[i].UserId + '" class="link">' + _.escape(dtoUserPage.Users[i].UserCode) + '</a></td>';
            html += '<td>' + _.escape(dtoUserPage.Users[i].FullName) + '</td>';
            html += '<td>' + _.escape(dtoUserPage.Users[i].JobTitleName) + '</td>';
            html += '<td>' + _.escape(dtoUserPage.Users[i].EMailId) + '</td>';
            html += '<td>' + _.escape(dtoUserPage.Users[i].LoginId) + '</td>';          
            if (dtoUserPage.Users[i].Status == 'Active') {
                html += '<td><label class="label label-success">' + _.escape(dtoUserPage.Users[i].Status) + '</label>' + '</td>';
            }
            else {
                html += '<td><label class="label label-danger">' + _.escape(dtoUserPage.Users[i].Status) + '</label>' + '</td>';
            }            
            html += ' <td class="td-center"><a href="javascript:void(0)" onclick=\"EditRecord(' + dtoUserPage.Users[i].UserId + ')\" class="link"><i class="fa fa-pencil" aria-hidden="true"></i></a>&nbsp;&nbsp;&nbsp;<a href="javascript:void(0)" onclick=\"DeleteRecord(' + dtoUserPage.Users[i].UserId + ')\" class="link"><i class="fa fa-trash" aria-hidden="true"></i></a></td>';
            html += '</tr>';
        }
        $('#bodyUsers').html(html);
        if (dtoUserPage.TotalRecords > 0) {
...
function NewRecord() {
    window.location = '/Admin/Main.html#User';
}

function EditRecord(UserId) {
    window.location = "/Admin/Main.html#User:" + UserId;
}

function Delete(UserId) {
    var params = { UserId: UserId };
    CallService('Admin/UserService.svc', 'Delete', params, OnUserDeleteSuccess, false);   
}
    ...
    bootbox.dialog({
        ...
        buttons: {
            ok: {
                ...
                callback: function () {
                    var params = { UserId: UserId };
                    var Deletable = CallService1('Admin/UserService.svc', 'Deletable', params);
                    if (Deletable == true) {
                        Delete(UserId);
                        $("#txtCurrentPageIndex").val('1');
                        FillGrid();
    ...
    bootbox.dialog({
        ...
        buttons: {
            ok: {
                ...
                className: "btn btn-primary",
                callback: function () {
                    $('#bodyUsers').find('tr').each(function () {
                        var row = $(this);
...
FillGrid();
socket.on('GetUserPage', function () {
    FillGrid();
})
