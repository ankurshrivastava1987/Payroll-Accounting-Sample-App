...
function FillGrid() {    
    ...
    CallService('Client/Contractor/ContractorService.svc', 'GetPage', params, OnContractorGetPageSuccess, true);
}
function OnContractorGetPageSuccess(dtoContractorPage) {
    if (dtoContractorPage != undefined) {
        $('#txtCurrentRecordCount').val(dtoContractorPage.Contractors.length);
        $("#txtTotalRecords").val(_.escape(dtoContractorPage.TotalRecords));
        var html = '';
        for (var i = 0; i < dtoContractorPage.Contractors.length; i++) {
            html += '<tr id="' + _.escape(dtoContractorPage.Contractors[i].ContractorId) + '" class="' + (i % 2 == 0 ? "footable-even" : "footable-odd") + '">';
            html += '<td><input type="checkbox" name="chk"/></td>';
            html += '<td></td>';
            html += '<td>' + _.escape(dtoContractorPage.Contractors[i].Type == 1 ? "Business" : "Individual") + '</td>';
            html += '<td><a href="/Client/Main.html#ViewContractor:' + _.escape(dtoContractorPage.Contractors[i].ContractorId) + '" class="link">' + _.escape(dtoContractorPage.Contractors[i].FederalEIN) + '</a></td>';
            html += '<td>' + _.escape(dtoContractorPage.Contractors[i].Line1) + '</td>';          
            html += '<td>' + _.escape(dtoContractorPage.Contractors[i].CityName) + '</td>';
            html += '<td>' + _.escape(dtoContractorPage.Contractors[i].StateName) + '</td>'; 
            html += ' <td class="td-center"><a href="javascript:void(0)" onclick=\"EditRecord(' + _.escape(dtoContractorPage.Contractors[i].ContractorId) + ')\" class="link"><i class="fa fa-pencil" aria-hidden="true"></i></a>&nbsp;&nbsp;&nbsp;<a href="javascript:void(0)" onclick=\"DeleteRecord(' + _.escape(dtoContractorPage.Contractors[i].ContractorId) + ')\" class="link"><i class="fa fa-trash" aria-hidden="true"></i></a></td>';
            html += '</tr>';
        }
        $('#bodyContractors').html(html);
        if (dtoContractorPage.TotalRecords > 0) {
...
function NewRecord() {
    window.location = '/Client/Main.html#Contractor';
}

function EditRecord(ContractorId) {
    window.location = '/Client/Main.html#Contractor:' + ContractorId;
}

function Delete(ContractorId) {
    var params = { ClientId: $.localStorage.get('ClientId'), ContractorId: ContractorId };
    CallService('Client/Contractor/ContractorService.svc', 'Delete', params, OnContractorDeleteSuccess, false);
}
function OnContractorDeleteSuccess(response) {

}
function DeleteRecord(ContractorId) {
    bootbox.dialog({
        ...
        buttons: {
            ok: {
                ...
                callback: function () {
                    var params = { ClientId: $.localStorage.get('ClientId'), ContractorId: ContractorId };
                    var Deletable = CallService1('Client/Contractor/ContractorService.svc', 'Deletable', params);
                    if (Deletable == true) {
                        Delete(ContractorId);
                        $("#txtCurrentPageIndex").val('1');
                        FillGrid();
    ...
    bootbox.dialog({
        ...
        buttons: {
            ok: {
                ...
                className: "btn btn-primary",
                callback: function () {
                    $('#bodyContractors').find('tr').each(function () {
                        var row = $(this);
...
FillGrid();
window.scrollTo(0, 0);
socket.on('GetContractorPage', function () {
    FillGrid();
})
