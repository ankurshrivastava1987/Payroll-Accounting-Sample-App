function FillGrid() {    
    ...
    CallService('Admin/StateTaxSetupService.svc', 'GetPage', params, OnStateTaxSetupGetPageSuccess, true);
}
function OnStateTaxSetupGetPageSuccess(dtoStateTaxSetupPage) {    
    if (dtoStateTaxSetupPage != undefined) {
        $('#txtCurrentRecordCount').val(dtoStateTaxSetupPage.StateTaxSetups.length);
        $("#txtTotalRecords").val(dtoStateTaxSetupPage.TotalRecords);
        var html = '';
        for (var i = 0; i < dtoStateTaxSetupPage.StateTaxSetups.length; i++) {
            html += '<tr id="' + dtoStateTaxSetupPage.StateTaxSetups[i].StateTaxSetupId + '" class="' + (i % 2 == 0 ? "footable-even" : "footable-odd") + '">';  
            html += '<td><input type="checkbox" name="chk"/></td>';
            html += '<td><a href="/Admin/Main.html#ViewStateTaxSetup:' + dtoStateTaxSetupPage.StateTaxSetups[i].StateTaxSetupId + '" class="link">' + _.escape(dtoStateTaxSetupPage.StateTaxSetups[i].StateName) + '</a></td>';
            html += '<td>' + FormatDateUTC(dtoStateTaxSetupPage.StateTaxSetups[i].EffectiveDate) + '</td>';
            html += '<td>' + _.escape(dtoStateTaxSetupPage.StateTaxSetups[i].PayScheduleRecurrenceName) + '</td>';
            html += '<td>' + _.escape(dtoStateTaxSetupPage.StateTaxSetups[i].FilingStatusName) + '</td>';                                    
            html += ' <td class="td-center"><a href="javascript:void(0)" onclick=\"EditRecord(' + dtoStateTaxSetupPage.StateTaxSetups[i].StateTaxSetupId + ')\" class="link"><i class="fa fa-pencil" aria-hidden="true"></i></a>&nbsp;&nbsp;&nbsp;<a href="javascript:void(0)" onclick=\"DeleteRecord(' + dtoStateTaxSetupPage.StateTaxSetups[i].StateTaxSetupId + ')\" class="link"><i class="fa fa-trash" aria-hidden="true"></i></a></td>';
            html += '</tr>';
        }
        $('#bodyStateTaxSetups').html(html);
        if (dtoStateTaxSetupPage.TotalRecords > 0) {
...
function NewRecord() {
    window.location = '/Admin/Main.html#StateTaxSetup';
}

function EditRecord(StateTaxSetupId) {
    window.location = '/Admin/Main.html#StateTaxSetup:' + StateTaxSetupId;
}

function Delete(StateTaxSetupId) {
    var params = { StateTaxSetupId: StateTaxSetupId };
    CallService('Admin/StateTaxSetupService.svc', 'Delete', params, OnStateTaxSetupDeleteSuccess, false);   
}
function OnStateTaxSetupDeleteSuccess(response) { 
    ...
    bootbox.dialog({
        ...
        buttons: {
            ok: {
                ...
                callback: function () {
                    var params = { StateTaxSetupId: StateTaxSetupId };
                    var Deletable = CallService1('Admin/StateTaxSetupService.svc', 'Deletable', params);
                    if (Deletable == true) {
...
FillGrid();
socket.on('GetStateTaxSetupPage', function () {
    FillGrid();
})

$('#aDownload').show();
$('#aDownload').attr('href', '/templates/StateTaxUploadTemplate.xlsx');
$('#aUpload').attr('data-bb', 'StateTaxSetup_html');
$('#aUpload').show();