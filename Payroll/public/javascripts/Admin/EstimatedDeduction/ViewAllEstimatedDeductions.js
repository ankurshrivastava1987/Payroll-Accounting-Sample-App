function FillGrid() {    
    ...
    CallService('Admin/EstimatedDeductionService.svc', 'GetPage', params, OnEstimatedDeductionGetPageSuccess, true);
}
function OnEstimatedDeductionGetPageSuccess(dtoEstimatedDeductionPage) {    
    if (dtoEstimatedDeductionPage != undefined) {
        $('#txtCurrentRecordCount').val(dtoEstimatedDeductionPage.EstimatedDeductions.length);
        $("#txtTotalRecords").val(dtoEstimatedDeductionPage.TotalRecords);
        var html = '';
        for (var i = 0; i < dtoEstimatedDeductionPage.EstimatedDeductions.length; i++) {
            html += '<tr id="' + dtoEstimatedDeductionPage.EstimatedDeductions[i].EstimatedDeductionId + '" class="' + (i % 2 == 0 ? "footable-even" : "footable-odd") + '">';  
            html += '<td><input type="checkbox" name="chk"/></td>';
            html += '<td><a href="/Admin/Main.html#ViewEstimatedDeduction:' + dtoEstimatedDeductionPage.EstimatedDeductions[i].EstimatedDeductionId + '" class="link">' + _.escape(dtoEstimatedDeductionPage.EstimatedDeductions[i].StateName) + '</a></td>';
            html += '<td>' + FormatDateUTC(dtoEstimatedDeductionPage.EstimatedDeductions[i].EffectiveDate) + '</td>';                                  
            html += ' <td class="td-center"><a href="javascript:void(0)" onclick=\"EditRecord(' + dtoEstimatedDeductionPage.EstimatedDeductions[i].EstimatedDeductionId + ')\" class="link"><i class="fa fa-pencil" aria-hidden="true"></i></a>&nbsp;&nbsp;&nbsp;<a href="javascript:void(0)" onclick=\"DeleteRecord(' + dtoEstimatedDeductionPage.EstimatedDeductions[i].EstimatedDeductionId + ')\" class="link"><i class="fa fa-trash" aria-hidden="true"></i></a></td>';
            html += '</tr>';
        }
        $('#bodyEstimatedDeductions').html(html);
        if (dtoEstimatedDeductionPage.TotalRecords > 0) {
...
function NewRecord() {
    window.location = '/Admin/Main.html#EstimatedDeduction';
}

function EditRecord(EstimatedDeductionId) {
    window.location = '/Admin/Main.html#EstimatedDeduction:' + EstimatedDeductionId;
}

function Delete(EstimatedDeductionId) {
    var params = { EstimatedDeductionId: EstimatedDeductionId };
    CallService('Admin/EstimatedDeductionService.svc', 'Delete', params, OnEstimatedDeductionDeleteSuccess, false);   
}
function OnEstimatedDeductionDeleteSuccess(response) { 

}
function DeleteRecord(EstimatedDeductionId) {
    bootbox.dialog({
        ...
        buttons: {
            ok: {
                ...
                callback: function () {
                    var params = { EstimatedDeductionId: EstimatedDeductionId };
                    var Deletable = CallService1('Admin/EstimatedDeductionService.svc', 'Deletable', params);
                    if (Deletable == true) {
                        Delete(EstimatedDeductionId);
                        $("#txtCurrentPageIndex").val('1');
                        FillGrid();
                    }
                    else {
    ...
    bootbox.dialog({
        ...
        buttons: {
            ok: {
                ...
                className: "btn btn-primary",
                callback: function () {
                    $('#bodyEstimatedDeductions').find('tr').each(function () {
                        var row = $(this);
...
FillGrid();
socket.on('GetEstimatedDeductionPage', function () {
    FillGrid();
})

$('#aDownload').show();
$('#aDownload').attr('href', '/templates/EstimatedDeductionTemplate.xlsx');
$('#aUpload').attr('data-bb', 'EstimatedDeduction_html');
$('#aUpload').show();