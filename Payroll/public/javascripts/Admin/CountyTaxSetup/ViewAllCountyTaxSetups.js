function FillGrid() {   
    ...
    CallService('Admin/CountyTaxSetupService.svc', 'GetPage', params, OnCountyTaxSetupGetPageSuccess, true);
}
function OnCountyTaxSetupGetPageSuccess(dtoCountyTaxSetupPage) {    
    if (dtoCountyTaxSetupPage != undefined) {
        $('#txtCurrentRecordCount').val(dtoCountyTaxSetupPage.CountyTaxSetups.length);
        $("#txtTotalRecords").val(dtoCountyTaxSetupPage.TotalRecords);
        var html = '';
        for (var i = 0; i < dtoCountyTaxSetupPage.CountyTaxSetups.length; i++) {
            html += '<tr id="' + dtoCountyTaxSetupPage.CountyTaxSetups[i].CountyTaxSetupId + '" class="' + (i % 2 == 0 ? "footable-even" : "footable-odd") + '">';  
            html += '<td><input type="checkbox" name="chk"/></td>';
            html += '<td><a href="/Admin/Main.html#ViewCountyTaxSetup:' + dtoCountyTaxSetupPage.CountyTaxSetups[i].CountyTaxSetupId + '" class="link">' + _.escape(dtoCountyTaxSetupPage.CountyTaxSetups[i].StateName) + '</a></td>';
            html += '<td>' + FormatDateUTC(dtoCountyTaxSetupPage.CountyTaxSetups[i].EffectiveDate) + '</td>';                                   
            html += ' <td class="td-center"><a href="javascript:void(0)" onclick=\"EditRecord(' + dtoCountyTaxSetupPage.CountyTaxSetups[i].CountyTaxSetupId + ')\" class="link"><i class="fa fa-pencil" aria-hidden="true"></i></a>&nbsp;&nbsp;&nbsp;<a href="javascript:void(0)" onclick=\"DeleteRecord(' + dtoCountyTaxSetupPage.CountyTaxSetups[i].CountyTaxSetupId + ')\" class="link"><i class="fa fa-trash" aria-hidden="true"></i></a></td>';
            html += '</tr>';
        }
        $('#bodyCountyTaxSetups').html(html);
        if (dtoCountyTaxSetupPage.TotalRecords > 0) {
...
function NewRecord() {
    window.location = '/Admin/Main.html#CountyTaxSetup';
}

function EditRecord(CountyTaxSetupId) {
    window.location = '/Admin/Main.html#CountyTaxSetup:' + CountyTaxSetupId;
}

function Delete(CountyTaxSetupId) {
    var params = { CountyTaxSetupId: CountyTaxSetupId };
    CallService('Admin/CountyTaxSetupService.svc', 'Delete', params, OnCountyTaxSetupDeleteSuccess, false);   
}
function OnCountyTaxSetupDeleteSuccess(response) { 

}
function DeleteRecord(CountyTaxSetupId) {
    bootbox.dialog({
        ...
        buttons: {
            ok: {
                ...
                callback: function () {
                    var params = { CountyTaxSetupId: CountyTaxSetupId };
                    var Deletable = CallService1('Admin/CountyTaxSetupService.svc', 'Deletable', params);
                    if (Deletable == true) {
                        Delete(CountyTaxSetupId);
                        $("#txtCurrentPageIndex").val('1');
                        FillGrid();
                    }
                    else {
    ...
    bootbox.dialog({
        ...
        buttons: {
            ok: {
                ...
                className: "btn btn-primary",
                callback: function () {
                    $('#bodyCountyTaxSetups').find('tr').each(function () {
                        var row = $(this);
...
FillGrid();
socket.on('GetCountyTaxSetupPage', function () {
    FillGrid();
})

$('#aDownload').show();
$('#aDownload').attr('href', '/templates/CountyTaxSetupTemplate.xlsx');
$('#aUpload').attr('data-bb', 'CountyTaxSetup_html');
$('#aUpload').show();