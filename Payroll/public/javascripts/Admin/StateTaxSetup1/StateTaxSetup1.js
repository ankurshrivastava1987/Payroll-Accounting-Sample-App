app.page("StateTaxSetup1", function () {
    return function (params) {
        if (parseInt(params, 10) > 0) {
            $('#hdnStateTaxSetupId').val(params);
            $('.page-head').html("Edit State Tax Setup 1");
            DisplayData();
            FillStateTaxSetup1Detail();
        }        
    }
});
$('#txtSearch').hide();
...
function DisplayData() {
    ...
    var dtoStateTaxSetup1 = CallService1('Admin/StateTaxSetup1Service.svc', 'GetObject', params);
    if (dtoStateTaxSetup1.StateTaxSetupId != undefined) {
        $('#cmbStateName').val(dtoStateTaxSetup1.StateId);
        $('#txtEffectiveDate').val(FormatDateUTC(dtoStateTaxSetup1.EffectiveDate));
        $('#cmbPayScheduleRecurrenceName').val(dtoStateTaxSetup1.PayScheduleRecurrenceId);
        $('#txtSubtractForEachAllowanceClaimed').val(dtoStateTaxSetup1.SubtractForEachAllowanceClaimed.toFixed(2));               
    }
    else {
...
function FillStateTaxSetup1Detail() {
    var params = { StateTaxSetupId: parseInt($('#hdnStateTaxSetupId').val(), 10) };
    CallService('Admin/StateTaxSetup1Service.svc', 'GetStateTaxSetup1Detail', params, OnGetStateTaxSetup1DetailSuccess, true);
}

function OnGetStateTaxSetup1DetailSuccess(response) {
    var i = 0;
    var StateTaxSetup1Details = '';
    $.each(response, function () {
        StateTaxSetup1Details += i + String.fromCharCode(134);        
        StateTaxSetup1Details += this.LowLimit.toFixed(2) + String.fromCharCode(134);
        StateTaxSetup1Details += this.HighLimit.toFixed(2) + String.fromCharCode(134);
        StateTaxSetup1Details += this.AC0.toFixed(2) + String.fromCharCode(134);
        StateTaxSetup1Details += this.AC1.toFixed(2) + String.fromCharCode(134);  
        StateTaxSetup1Details += this.AC2.toFixed(2) + String.fromCharCode(134);             
        StateTaxSetup1Details += this.AC3.toFixed(2) + String.fromCharCode(134);  
        StateTaxSetup1Details += this.AC4.toFixed(2) + String.fromCharCode(134);  
        StateTaxSetup1Details += this.AC5.toFixed(2) + String.fromCharCode(134);  
        StateTaxSetup1Details += this.AC6.toFixed(2) + String.fromCharCode(134);  
        StateTaxSetup1Details += this.AC7.toFixed(2) + String.fromCharCode(134);  
        StateTaxSetup1Details += this.AC8.toFixed(2) + String.fromCharCode(134);  
        StateTaxSetup1Details += this.AC9.toFixed(2) + String.fromCharCode(134);          
        StateTaxSetup1Details += this.AC10.toFixed(2) + String.fromCharCode(134);  
        StateTaxSetup1Details += "0";
        if (i < response.length - 1) {
            StateTaxSetup1Details += String.fromCharCode(135);
        }
        i++;
    });
    $('#hdnStateTaxSetup1Details').val(StateTaxSetup1Details);
    FillGrid();    
}
...
function Save() {
    if (form.valid()) {       
        var params = {StateTaxSetupId: parseInt($("#hdnStateTaxSetupId").val(), 10), StateId: parseInt($("#cmbStateName").val(), 10), EffectiveDate: $("#txtEffectiveDate").val(), PayScheduleRecurrenceId: parseInt($("#cmbPayScheduleRecurrenceName").val(), 10), SubtractForEachAllowanceClaimed: parseFloat($('#txtSubtractForEachAllowanceClaimed').val()), StateTaxSetup1Details: $("#hdnStateTaxSetup1Details").val() };
        CallService('Admin/StateTaxSetup1Service.svc', 'Save', params, OnStateTaxSetup1SaveSuccess, false);
    }
};

function OnStateTaxSetup1SaveSuccess(StateTaxSetupId) {
...
$(document).ready(function () {    
    ...
    $("#btnOK").click(function (e) {
        if (form1.valid()) {
            var arrDetails = $('#hdnStateTaxSetup1Details').val().split(String.fromCharCode(135));
            if (arrDetails[0] == "") {
            ...
            $('#hdnStateTaxSetup1Details').val(arrDetails.join(String.fromCharCode(135)));
            FillGrid();
...
function DeleteSelected() {
    ...
    $('#tblStateTaxSetup1Details').find('tr').each(function () {
        var row = $(this);
...
function FillGrid() {
    var html = '';   
    if ($('#hdnStateTaxSetup1Details').val() != "") {
        var arrDetails = $('#hdnStateTaxSetup1Details').val().split(String.fromCharCode(135));
