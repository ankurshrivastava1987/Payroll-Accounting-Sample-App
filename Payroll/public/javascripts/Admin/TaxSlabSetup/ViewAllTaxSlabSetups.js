function FillGrid() {   
    ...
    CallService('Admin/TaxSlabSetupService.svc', 'GetPage', params, OnTaxSlabSetupGetPageSuccess, true);
}
function OnTaxSlabSetupGetPageSuccess(dtoTaxSlabSetupPage) {    
    if (dtoTaxSlabSetupPage != undefined) {
        $('#txtCurrentRecordCount').val(dtoTaxSlabSetupPage.TaxSlabSetups.length);
        $("#txtTotalRecords").val(dtoTaxSlabSetupPage.TotalRecords);
        var html = '';
        for (var i = 0; i < dtoTaxSlabSetupPage.TaxSlabSetups.length; i++) {
            html += '<tr id="' + dtoTaxSlabSetupPage.TaxSlabSetups[i].TaxSlabSetupId + '" class="' + (i % 2 == 0 ? "footable-even" : "footable-odd") + '">';  
            html += '<td><input type="checkbox" name="chk"/></td>';
            html += '<td><a href="/Admin/Main.html#ViewTaxSlabSetup:' + dtoTaxSlabSetupPage.TaxSlabSetups[i].TaxSlabSetupId + '" class="link">' + _.escape(dtoTaxSlabSetupPage.TaxSlabSetups[i].StateName) + '</a></td>';
            html += '<td>' + FormatDateUTC(dtoTaxSlabSetupPage.TaxSlabSetups[i].EffectiveDate) + '</td>';                                   
            html += ' <td class="td-center"><a href="javascript:void(0)" onclick=\"EditRecord(' + dtoTaxSlabSetupPage.TaxSlabSetups[i].TaxSlabSetupId + ')\" class="link"><i class="fa fa-pencil" aria-hidden="true"></i></a>&nbsp;&nbsp;&nbsp;<a href="javascript:void(0)" onclick=\"DeleteRecord(' + dtoTaxSlabSetupPage.TaxSlabSetups[i].TaxSlabSetupId + ')\" class="link"><i class="fa fa-trash" aria-hidden="true"></i></a></td>';
            html += '</tr>';
        }
        $('#bodyTaxSlabSetups').html(html);
        if (dtoTaxSlabSetupPage.TotalRecords > 0) {
...
function NewRecord() {
    window.location = '/Admin/Main.html#TaxSlabSetup';
}

function EditRecord(TaxSlabSetupId) {
    window.location = '/Admin/Main.html#TaxSlabSetup:' + TaxSlabSetupId;
}

function Delete(TaxSlabSetupId) {
    var params = { TaxSlabSetupId: TaxSlabSetupId };
    CallService('Admin/TaxSlabSetupService.svc', 'Delete', params, OnTaxSlabSetupDeleteSuccess, false);   
}
function OnTaxSlabSetupDeleteSuccess(response) { 

}
function DeleteRecord(TaxSlabSetupId) {
    bootbox.dialog({
        ...
        buttons: {
            ok: {
                ...
                callback: function () {
                    var params = { TaxSlabSetupId: TaxSlabSetupId };
                    var Deletable = CallService1('Admin/TaxSlabSetupService.svc', 'Deletable', params);
                    if (Deletable == true) {
                        Delete(TaxSlabSetupId);
                        $("#txtCurrentPageIndex").val('1');
                        FillGrid();
                    }
                    else {
    ...
    bootbox.dialog({
        ...
        buttons: {
            ok: {
                ...
                className: "btn btn-primary",
                callback: function () {
                    $('#bodyTaxSlabSetups').find('tr').each(function () {
                        var row = $(this);
...
FillGrid();
socket.on('GetTaxSlabSetupPage', function () {
    FillGrid();
})

$('#aDownload').show();
$('#aDownload').attr('href', '/templates/TaxSlabSetupTemplate.xlsx');
$('#aUpload').attr('data-bb', 'TaxSlabSetup_html');
$('#aUpload').show();