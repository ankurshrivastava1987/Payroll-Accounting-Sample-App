...
function OnEmployeeGetPageSuccess(dtoEmployeePage) {    
    if (dtoEmployeePage != undefined) {
        ...
        for (var i = 0; i < dtoEmployeePage.Employees.length; i++) {
            html += '<tr id="' + _.escape(dtoEmployeePage.Employees[i].ClientId) + '" class="' + (i % 2 == 0 ? "footable-even" : "footable-odd") + '">';
            html += '<td><input type="checkbox" name="chk"/></td>';
            html += '<td style="vertical-align:middle"><div class="' + (dtoEmployeePage.Employees[i].Status == "Active" ? "green-circle" : "red-circle") + '"></div></td>';
            html += '<td><a href="/Client/Main.html#ViewEmployee:' + _.escape(dtoEmployeePage.Employees[i].EmployeeId) + '" class="link">' + _.escape(dtoEmployeePage.Employees[i].FullName) + '</a></td>';            
            html += '<td>' + _.escape(dtoEmployeePage.Employees[i].StateCode) + '</td>';
            ...
            html += '<td class="td-center"><a href="javascript:void(0)" onclick=\"EditRecord(' + _.escape(dtoEmployeePage.Employees[i].EmployeeId) + ')\" class="link"><i class="fa fa-pencil" aria-hidden="true"></i></a>&nbsp;&nbsp;&nbsp;<a href="javascript:void(0)" onclick=\"DeleteRecord(' + _.escape(dtoEmployeePage.Employees[i].EmployeeId) + ')\" class="link"><i class="fa fa-trash" aria-hidden="true"></i></a></td>';
            html += '</tr>';
        }
...
function NewRecord() {
    window.location = '/Client/Main.html#Employee';
}

function EditRecord(EmployeeId) {
    window.location = '/Client/Main.html#Employee:' + EmployeeId;
}

function Delete(EmployeeId) {
    var params = { ClientId: $.localStorage.get('ClientId'), EmployeeId: EmployeeId };
    CallService('Client/EmployeeService.svc', 'Delete', params, OnEmployeeDeleteSuccess, false);   
}
function OnEmployeeDeleteSuccess(response) { 
    ...
    bootbox.dialog({
        ...
        buttons: {
            ok: {
                ...
                callback: function () {
                    var params = { ClientId: $.localStorage.get('ClientId'), EmployeeId: EmployeeId };
                    var Deletable = CallService1('Client/EmployeeService.svc', 'Deletable', params);
                    if (Deletable == true) {
                        Delete(EmployeeId);
                        $("#txtCurrentPageIndex").val('1');
                        FillGrid();
    ...
    bootbox.dialog({
        ...
        buttons: {
            ok: {
                ...
                className: "btn btn-primary",
                callback: function () {
                    $('#bodyEmployees').find('tr').each(function () {
                        var row = $(this);
...
$('#aDownload').show();
$('#aDownload').attr('href', '/templates/EmployeeUploadTemplate.xlsx');
$('#aUpload').attr('data-bb', 'EmployeeUpload_html');
$('#aUpload').show();
