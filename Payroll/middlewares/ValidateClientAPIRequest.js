        ...
        try {
            ...
            var connection = new sql.Connection(Configuration.ConnectionString, function (err) {
                if (err) {
                    ...
                    request.input('ClientId', sql.BigInt, key);
                    request.execute('prcClientValidateByClientIdGet', function (err, recordsets, returnValue, affected) {
                        connection.close();
                        if (err) {
                            winston.log('error', err);
                        }
                            ...
                            try {
                                if (recordsets[0].length > 0) {
                                    ...
                                    //if (dtoUser) {
                                    if (req.url.toLowerCase().indexOf('/payroll.services/client') >= 0) {
