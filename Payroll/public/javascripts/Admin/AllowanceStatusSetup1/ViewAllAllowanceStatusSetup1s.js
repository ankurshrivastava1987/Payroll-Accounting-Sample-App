function FillGrid() {    
    ...
    CallService('Admin/AllowanceStatusSetup1Service.svc', 'GetPage', params, OnAllowanceStatusSetup1GetPageSuccess, true);
}
function OnAllowanceStatusSetup1GetPageSuccess(dtoAllowanceStatusSetup1Page) {    
    if (dtoAllowanceStatusSetup1Page != undefined) {
        $('#txtCurrentRecordCount').val(dtoAllowanceStatusSetup1Page.AllowanceStatusSetup1s.length);
        $("#txtTotalRecords").val(dtoAllowanceStatusSetup1Page.TotalRecords);
        var html = '';
        for (var i = 0; i < dtoAllowanceStatusSetup1Page.AllowanceStatusSetup1s.length; i++) {
            html += '<tr id="' + dtoAllowanceStatusSetup1Page.AllowanceStatusSetup1s[i].AllowanceStatusSetupId + '" class="' + (i % 2 == 0 ? "footable-even" : "footable-odd") + '">';  
            html += '<td><input type="checkbox" name="chk"/></td>';
            html += '<td><a href="/Admin/Main.html#ViewAllowanceStatusSetup1:' + dtoAllowanceStatusSetup1Page.AllowanceStatusSetup1s[i].AllowanceStatusSetupId + '" class="link">' + _.escape(dtoAllowanceStatusSetup1Page.AllowanceStatusSetup1s[i].StateName) + '</a></td>';
            html += '<td>' + dtoAllowanceStatusSetup1Page.AllowanceStatusSetup1s[i].AllowancePercentage.toFixed(2) + '%</td>';
            html += '<td>' + FormatDateUTC(dtoAllowanceStatusSetup1Page.AllowanceStatusSetup1s[i].EffectiveDate) + '</td>';                                  
            html += ' <td class="td-center"><a href="javascript:void(0)" onclick=\"EditRecord(' + dtoAllowanceStatusSetup1Page.AllowanceStatusSetup1s[i].AllowanceStatusSetupId + ')\" class="link"><i class="fa fa-pencil" aria-hidden="true"></i></a>&nbsp;&nbsp;&nbsp;<a href="javascript:void(0)" onclick=\"DeleteRecord(' + dtoAllowanceStatusSetup1Page.AllowanceStatusSetup1s[i].AllowanceStatusSetupId + ')\" class="link"><i class="fa fa-trash" aria-hidden="true"></i></a></td>';
            html += '</tr>';
        }
        $('#bodyAllowanceStatusSetup1s').html(html);
        if (dtoAllowanceStatusSetup1Page.TotalRecords > 0) {
...
function NewRecord() {
    window.location = '/Admin/Main.html#AllowanceStatusSetup1';
}

function EditRecord(AllowanceStatusSetupId) {
    window.location = '/Admin/Main.html#AllowanceStatusSetup1:' + AllowanceStatusSetupId;
}

function Delete(AllowanceStatusSetupId) {
    var params = { AllowanceStatusSetupId: AllowanceStatusSetupId };
    CallService('Admin/AllowanceStatusSetup1Service.svc', 'Delete', params, OnAllowanceStatusSetup1DeleteSuccess, false);   
}
function OnAllowanceStatusSetup1DeleteSuccess(response) { 

}
function DeleteRecord(AllowanceStatusSetupId) {
    bootbox.dialog({
        ...
        buttons: {
            ok: {
                ...
                callback: function () {
                    var params = { AllowanceStatusSetupId: AllowanceStatusSetupId };
                    var Deletable = CallService1('Admin/AllowanceStatusSetup1Service.svc', 'Deletable', params);
                    if (Deletable == true) {
                        Delete(AllowanceStatusSetupId);
                        $("#txtCurrentPageIndex").val('1');
                        FillGrid();
                    }
                    else {
    ...
    bootbox.dialog({
        ...
        buttons: {
            ok: {
                ...
                className: "btn btn-primary",
                callback: function () {
                    $('#bodyAllowanceStatusSetup1s').find('tr').each(function () {
                        var row = $(this);
...
FillGrid();
socket.on('GetAllowanceStatusSetup1Page', function () {
    FillGrid();
})

$('#aDownload').show();
$('#aDownload').attr('href', '/templates/AllowanceStatusSetup1Template.xlsx');
$('#aUpload').attr('data-bb', 'AllowanceStatusSetup1_html');
$('#aUpload').show();